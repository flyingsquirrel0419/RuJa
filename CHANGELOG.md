...
- Added Realm-local static `Temporal.PlainMonthDay.from` and
  ...
  field order, parse audited partial/full date strings, apply constrain/reject
  overflow, ignore their receiver, and allocate through the method Realm.
  MonthDay uses an explicit input year only for overflow before storing the
  ...
  - 기존 구현 및 제약 조건: constructor/accessor core는 있었지만 from이 없었고 helper assertion은 별도 미구현 toString을 호출해 factory 성공과 formatting 실패를 함께 보고했다.
...
- Added Realm-local `%Temporal.PlainMonthDay%` and
  ...
  blocked on the intentionally separate sibling `from` factories, and the
  installer reserved 165 maximum live pins across 166 allocations. The later
  factory unit above resolves that blocker and extends the installer.
...
- Added Realm-local `%Temporal.PlainTime%` with immutable six-field hidden
  ...
  `toString`, hidden-record `toJSON`, and the always-throwing `valueOf`.
  ...
  Static `compare` converts both operands completely from left to right, then
  lexicographically compares all six copied fields and returns `-1`, `+0`, or
  `1` without allocating a result object.
  Prototype `toString` brands before options, reads
  `fractionalSecondDigits`, `roundingMode`, and `smallestUnit` in specification
  order, and uses the shared nanosecond formatter for auto/fixed precision,
  all rounding modes, and midnight rollover without observing public fields.
  Prototype `toJSON` ignores every argument and serializes the same copied
  hidden record at automatic precision without dispatching through a mutable
  `toString` property. Prototype `round` brands before its required argument,
  accepts String shorthand or an options object, observes
  `roundingIncrement`, `roundingMode`, and `smallestUnit` in specification
  order, and returns a fresh method-Realm PlainTime after exact i128 rounding.
  All six time units, valid divisors, nine modes, and midnight rollover are
...
- Added Realm-local static `Temporal.PlainDate.from`. It copies branded
  ...
  order; and parses the audited PlainDate String grammar without reading
  public Temporal accessors. Invalid primitives and syntactically invalid
  strings fail before options; parsed strings and prepared property fields
  observe `overflow` before date-range validation. Results always use the native function Realm. The pinned direct
  boundary is exact **70/0/0** and forced **70/1/0** over 71 files; the sole
  blocker requires absent PlainMonthDay and PlainYearMonth. The former 11
  PlainDate core blockers are now admitted, making that boundary **78/0/0**.

  [Decision Log]
  - 목적과 의도: PlainDate의 표준 conversion 경계를 hidden slots, property bag, String 입력에 대해 완결하고 accessor core의 factory 의존성을 제거한다.
  - 기존 구현 및 제약 조건: constructor와 ISO accessors는 있었지만 static from이 없었다. PlainDateTime용 collector는 시간 getter까지 읽어 PlainDate의 관찰 순서를 깨고, calendar sibling hidden kinds는 아직 없다.
  - 검토한 주요 대안: PlainDateTime.from 재사용, constructor 재호출, 11개 core 테스트용 stub, 날짜 전용 immutable conversion record를 검토했다.
  - 선택한 방식: branded 세 종류는 hidden slots로 날짜/calendar를 복사하고, ordinary object는 5-field 전용 collector와 overflow 후 resolver를 사용하며, String은 공용 저수준 ISO parser에서 날짜와 calendar만 추출한다.
  - 다른 대안 대신 이 방식을 선택한 이유: public accessor/constructor 재사용은 user code와 subclass를 관찰하고, stub은 intended assertion 이전 TypeError false positive를 만든다. 전용 collector만 명세 getter 순서와 시간 필드 비관찰을 동시에 보존한다.
  - 장점, 단점 및 영향: constrain/reject, leap second, offset/annotation 문법, UTC designator 거부, 극한 날짜, method Realm, fuel, GC/OOM과 115 pins/116 allocations가 검증된다. calendar-temporal-object 한 경로는 PlainMonthDay/PlainYearMonth 도입 후 재측정한다.
...
- Added Realm-local `%Temporal.PlainDate%` hidden-slot construction with a
  ...
  pinned core boundary is now exact **78/0/0** after `PlainDate.from` removed
  all 11 factory blockers. This releases four PlainDateTime.from paths, one equals path, and
  ...
  - 장점, 단점 및 영향: conversion order, 극한 날짜, distinct branding, hidden getter 비관찰, cross-Realm/subclass, fuel, GC/OOM rollback과 73개 신규 exact path가 검증된다. 초기 core installer는 114 pins/115 allocations였고 static from 추가 후 115/116이다. calendar sibling types는 후속 단위다.
...
- Added Realm-local `Temporal.ZonedDateTime.prototype.withTimeZone`. It brands
  ...
  - 기존 구현 및 제약 조건: hidden time-zone slot과 property-bag용 변환 helper는 있었지만 공개 `withTimeZone` 메서드와 method-Realm 결과 생성 경로가 없었다. installer pin 예약은 최대 live pin보다 작았고 `+00:00` kind 분류가 constructor/converter 사이에서 달랐다. named IANA transition backend와 다른 Temporal constructors는 아직 없다.
