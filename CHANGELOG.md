...
- Added Realm-local `%Temporal.PlainTime%` with immutable six-field hidden
  ...
  static `from` and `compare`, hidden-record `equals`, and the always-throwing
  `valueOf`.
  ...
  exact **161/0/0**: core **40**, `from` **51**, `valueOf` **7**, `equals`
  **31**, and `compare` **32**. The complete direct PlainDate
  ...
  **260/6** over 266 files. Installer accounting is 132 maximum live pins and
  133 allocations.
  ...
  - 선택한 방식: six-field compact hidden kind를 Realm별 constructor/prototype에 연결하고 constructor/accessor/from/equals/valueOf/compare와 bridge가 같은 validated record 변환을 사용한다. compare는 첫 operand 변환을 완결한 뒤 둘째를 변환하고 six-field tuple을 allocation 없이 비교한다. exact manifest는 core/from/valueOf/equals/compare를 분리하지만 하나의 metadata 모듈과 진단기로 공유한다.
  ...
  - 장점, 단점 및 영향: cross-Realm/newTarget, hidden getter 비관찰, conversion order, constrain/reject, fresh clone, zero-allocation equality/comparison, GC/OOM/root retry와 양 operand exact fuel이 한 경로로 검증된다. compare 추가는 기존 allocation 번호를 보존하며 allocation 131만 추가한다. add/subtract/round/since/until/with/string/locale 메서드는 후속 단위로 남는다.
