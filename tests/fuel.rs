...
#[test]
fn temporal_plain_year_month_equals_precharges_input_bytes() {
    const BUDGET: i64 = 10_000;
    let mut vm = Vm::new().expect("failed to initialize VM");
    vm.run(
        r#"
        globalThis.plainYearMonthEquals = new Temporal.PlainYearMonth(1970, 1);
        globalThis.plainYearMonthEqualsShort = "1970-01[foo=a]";
        globalThis.plainYearMonthEqualsLong = "1970-01[foo=" + "a".repeat(512) + "]";
        "#,
    )
    .expect("PlainYearMonth.equals fuel fixtures should initialize");

    vm.set_fuel(Some(BUDGET));
    vm.run("plainYearMonthEquals.equals(plainYearMonthEqualsShort);")
        .expect("short PlainYearMonth input should parse");
    let short_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");

    vm.set_fuel(Some(BUDGET));
    vm.run("plainYearMonthEquals.equals(plainYearMonthEqualsLong);")
        .expect("long PlainYearMonth input should parse");
    let long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
    assert!(long_work >= short_work + 500);

    vm.set_fuel(Some(long_work - 1));
    let error = vm
        .run("plainYearMonthEquals.equals(plainYearMonthEqualsLong);")
        .expect_err("N-1 fuel must abort before parsing");
    assert_eq!(error.kind, ruja::ErrorKind::Fuel);
    assert_eq!(vm.fuel_remaining(), Some(0));
...
fn temporal_plain_year_month_compare_precharges_each_input_string() {
    ...
    vm.run(
        r#"
        globalThis.yearMonthCompareShort = "1970-01[foo=a]";
        globalThis.yearMonthCompareLong = "1970-01[foo=" + "a".repeat(512) + "]";
        "#,
    )
    .expect("PlainYearMonth.compare fuel fixtures should initialize");

    vm.set_fuel(Some(BUDGET));
    vm.run("Temporal.PlainYearMonth.compare(yearMonthCompareShort, yearMonthCompareShort);")
        .expect("short PlainYearMonth inputs should parse");
    let short_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");

    vm.set_fuel(Some(BUDGET));
    vm.run("Temporal.PlainYearMonth.compare(yearMonthCompareLong, yearMonthCompareShort);")
        .expect("long first PlainYearMonth input should parse");
    let first_long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
    assert!(first_long_work >= short_work + 500);

    vm.set_fuel(Some(BUDGET));
    vm.run("Temporal.PlainYearMonth.compare(yearMonthCompareShort, yearMonthCompareLong);")
        .expect("long second PlainYearMonth input should parse");
    let second_long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
    assert!(second_long_work >= short_work + 500);

    vm.set_fuel(Some(BUDGET));
    vm.run("Temporal.PlainYearMonth.compare(yearMonthCompareLong, yearMonthCompareLong);")
        .expect("long PlainYearMonth inputs should parse");
    let long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
    assert!(long_work >= first_long_work + 500);
    assert!(long_work >= second_long_work + 500);

    vm.set_fuel(Some(long_work - 1));
    let error = vm
        .run("Temporal.PlainYearMonth.compare(yearMonthCompareLong, yearMonthCompareLong);")
        .expect_err("N-1 fuel must abort during comparison conversion");
    assert_eq!(error.kind, ruja::ErrorKind::Fuel);
    assert_eq!(vm.fuel_remaining(), Some(0));
...
#[test]
fn temporal_plain_month_day_to_plain_date_precharges_year_strings() {
    const BUDGET: i64 = 20_000;
    let mut vm = Vm::new().expect("failed to initialize VM");
    vm.run(
        r#"
        globalThis.monthDayToDateFuel = new Temporal.PlainMonthDay(5, 2);
        globalThis.monthDayToDateShort = { year: '1' };
        globalThis.monthDayToDateLong = { year: '0'.repeat(512) + '1' };
        "#,
    )
    .expect("PlainMonthDay.toPlainDate fuel fixtures should initialize");

    vm.set_fuel(Some(BUDGET));
    vm.run("monthDayToDateFuel.toPlainDate(monthDayToDateShort);")
        .expect("short year string should convert");
    let short_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");

    vm.set_fuel(Some(BUDGET));
    vm.run("monthDayToDateFuel.toPlainDate(monthDayToDateLong);")
        .expect("long year string should convert");
    let long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
    assert!(long_work >= short_work + 500);

    vm.set_fuel(Some(long_work - 1));
    let error = vm
        .run("monthDayToDateFuel.toPlainDate(monthDayToDateLong);")
        .expect_err("N-1 fuel must abort during year conversion");
    assert_eq!(error.kind, ruja::ErrorKind::Fuel);
    assert_eq!(vm.fuel_remaining(), Some(0));
...
#[test]
fn temporal_duration_to_string_precharges_option_strings() {
    const BUDGET: i64 = 20_000;
    let mut vm = Vm::new().expect("failed to initialize VM");
    vm.run(
        r#"
        globalThis.durationToStringFuel = new Temporal.Duration(1, 2, 3, 4, 5, 6, 7, 987, 650);
        globalThis.durationOptionShort = { toString() { return 'trunc'; } };
        globalThis.durationOptionLong = { toString() { return 'x'.repeat(512); } };
        "#,
    )
    .expect("Duration.toString fuel fixtures should initialize");

    for property in ["fractionalSecondDigits", "roundingMode", "smallestUnit"] {
        vm.set_fuel(Some(BUDGET));
        let _ = vm.run(&format!(
            "try {{ durationToStringFuel.toString({{ {property}: durationOptionShort }}); }} catch (error) {{}}"
        ));
        let short_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");

        vm.set_fuel(Some(BUDGET));
        let _ = vm.run(&format!(
            "try {{ durationToStringFuel.toString({{ {property}: durationOptionLong }}); }} catch (error) {{}}"
        ));
        let long_work = BUDGET - vm.fuel_remaining().expect("fuel should remain enabled");
        assert!(
            long_work >= short_work + 500,
