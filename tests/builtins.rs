...
#[test]
fn object_group_by_observes_iterator_record_close_priority_and_gc_roots() {
    let mut vm = Vm::new().expect("failed to initialize VM");
    vm.register_fn(
        "forceGc",
        |vm, _, _| {
    ...
    assert_eq!(
        vm.run(
            ...
            var descriptor = Object.getOwnPropertyDescriptor(grouped, "all");
            [
              effects.join(","), hasCalls, iteratorGets, nextGets,
              iteratorThis, iteratorArgs,
              nextThis, nextArgs.join(":"), callbackThis,
              callbackArgs.join(":"), grouped.all[0] === kept,
              Object.getPrototypeOf(grouped) === null,
              descriptor.writable, descriptor.enumerable, descriptor.configurable
            ].join("|");
            "#,
        )
        .expect("grouping should preserve direct iterator and GC roots"),
        Value::String(Arc::from(
            "iterator-get,iterator-call,next-get,next:0,callback:0,key:string,next:1|0|1|1|true|0|true|0:0|true|2:0|true|true|true|true|true"
        ))
    );
    ...
    assert_eq!(
        vm.run(
            r#"
            function closeCase(kind, keyFailure) {
              var original = {};
              var replacement = {};
              var calls = 0;
              var iterator = {
              ...
              if (kind === "null") {
                ...
                iterator.return = function() { calls++; forceGc(); return 1; };
              }
              var iterable = { [Symbol.iterator]: function() { return iterator; } };
              try {
                Object.groupBy(iterable, function() {
                  if (!keyFailure) throw original;
                  return { [Symbol.toPrimitive]: function() { throw original; } };
                });
              } catch (error) {
            ...
            [
              closeCase("absent", false), closeCase("null", false),
              closeCase("getter", false), closeCase("noncallable", false),
              closeCase("throw", false), closeCase("primitive", false),
              closeCase("throw", true)
            ].join(",");
            "#,
        )
        .expect("IteratorClose must preserve the original grouping error"),
        Value::String(Arc::from(
            "true:0,true:0,true:0,true:0,true:1,true:1,true:1"
        ))
    );
    ...
    assert_eq!(
        vm.run(
            ...
            function noCloseCase(kind) {
              ...
              try {
                var result = Object.groupBy(
                  { [Symbol.iterator]: function() { return iterator; } },
                  function() { return "k"; }
                );
                return (kind === "complete" && Object.keys(result).length === 0) +
                  ":" + closed;
              } catch (error) {
        ...
        .expect("IteratorStepValue failures must not close the iterator"),
    ...
    assert_eq!(
        vm.run(
            ...
            function groupedValue(iterable) {
              return Object.groupBy(iterable, function() { return "k"; }).k[0];
            }
            var array = ["native"];
            ...
            [
              groupedValue(array), groupedValue(map),
              groupedValue(set), groupedValue(generator)
            ].join(",");
            "#,
        )
        .expect("Object.groupBy must observe every overridden iterator method"),
...
#[test]
fn temporal_zoned_date_time_equals_compares_complete_hidden_identity() {
    assert_eq!(
        run(r#"
            var base = new Temporal.ZonedDateTime(0n, '+00');
            var same = new Temporal.ZonedDateTime(0n, '+0000');
            var utc = new Temporal.ZonedDateTime(0n, 'UTC');
            var later = new Temporal.ZonedDateTime(1n, '+00:00');
            var reads = 0;
            Object.defineProperty(same, 'epochNanoseconds', {
              get: function () { reads++; throw new Error('observed'); }
            });
            [
              base.equals(same),
              base.equals(utc),
              base.equals(later),
              base.equals({ year: 1970, month: 1, day: 1, timeZone: '+00:00' }),
              base.equals('1970-01-01T00:00Z[+00:00]'),
              utc.equals('1970-01-01[utC]'),
              base.equals('1970-01-01[+00:00]'),
              reads,
              Temporal.ZonedDateTime.prototype.equals.length,
              Temporal.ZonedDateTime.prototype.equals.name
            ].join('|');
        "#),
        Value::String(Arc::from("true|false|false|true|true|true|true|0|1|equals"))
    );

    for source in [
        "new Temporal.ZonedDateTime(0n, 'UTC').equals()",
        "Temporal.ZonedDateTime.prototype.equals.call({}, new Temporal.ZonedDateTime(0n, 'UTC'))",
        "new Temporal.ZonedDateTime.prototype.equals(new Temporal.ZonedDateTime(0n, 'UTC'))",
    ] {
        assert!(run_err(source).contains("TypeError"), "{source}");
    }
    ...
    assert_eq!(
        run(r#"
            var other = $262.createRealm().global;
            var method = other.Temporal.ZonedDateTime.prototype.equals;
            var main = new Temporal.ZonedDateTime(0n, 'UTC');
            var foreign = new other.Temporal.ZonedDateTime(0n, 'UTC');
            var receiverRealm;
            var argumentRealm;
            try { method.call({}, foreign); } catch (error) {
              receiverRealm = error instanceof other.TypeError && !(error instanceof TypeError);
            }
            try { method.call(foreign, {}); } catch (error) {
              argumentRealm = error instanceof other.TypeError && !(error instanceof TypeError);
            }
            [main.equals(foreign), method.call(main, foreign), receiverRealm, argumentRealm].join('|');
        "#),
        Value::String(Arc::from("true|true|true|true"))
    );
}
