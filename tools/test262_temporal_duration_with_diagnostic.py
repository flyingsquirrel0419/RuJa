#!/usr/bin/env python3
"""Force the frozen Temporal.Duration.prototype.with surface through RuJa."""

from pathlib import Path

import test262_runner

from test262_temporal_duration_with_admission import TEMPORAL_DURATION_WITH_FILES
...
def should_skip(meta, path=None):
    if path is not None and _relative(path) in TEMPORAL_DURATION_WITH_FILES:
        return False
...
def verify_expected_results(arguments):
    ...
    frozen = requested & TEMPORAL_DURATION_WITH_FILES
    test_root = Path(test262_runner.TEST262) / "test"
    actual = {path: test262_runner.run_test(test_root / path) for path in frozen}
    expected = {path: "pass" for path in frozen}
    if actual != expected:
        raise RuntimeError(f"Temporal.Duration.prototype.with results drifted: {actual}")
