#!/usr/bin/env python3
"""Force and identity-check all 29 admitted PlainDateTime.prototype.with tests."""
...
SURFACE = TEMPORAL_PLAIN_DATE_TIME_WITH_FILES
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError(
            "PlainDateTime with diagnostic requires the exact frozen 29-file surface"
        )
    ...
    if actual != expected:
        raise RuntimeError(f"PlainDateTime with results drifted: {actual}")
