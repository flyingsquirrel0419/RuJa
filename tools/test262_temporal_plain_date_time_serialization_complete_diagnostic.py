#!/usr/bin/env python3
"""Force the complete 64-file PlainDateTime serialization surface."""
...
SURFACE = TEMPORAL_PLAIN_DATE_TIME_SERIALIZATION_SURFACE
...
def _verify_corpus(test_root):
    for relative in sorted(SURFACE):
        ...
        if actual != expected:
            raise RuntimeError(
                f"complete PlainDateTime serialization metadata drifted: {relative}: {actual}"
            )
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError(
            "complete PlainDateTime serialization diagnostic requires the exact "
            "frozen 64-file surface"
        )
    ...
    if actual != expected:
        raise RuntimeError(f"complete PlainDateTime serialization results drifted: {actual}")
    wrong_errors = {
        path: messages
    ...
    if wrong_errors:
        raise RuntimeError(
            "complete PlainDateTime serialization failure reasons drifted: "
            f"{wrong_errors}"
        )
