...
import test262_temporal_calendar_siblings_formatter_surface_diagnostic as temporal_sibling_formatter_diagnostic
import test262_temporal_calendar_siblings_to_string_diagnostic as temporal_sibling_to_string_diagnostic
import test262_temporal_calendar_siblings_with_diagnostic as temporal_sibling_with_diagnostic
import test262_temporal_plain_month_day_to_json_diagnostic as temporal_month_day_to_json_diagnostic
...
from test262_temporal_plain_month_day_to_plain_date_intl_admission import (
...
from test262_temporal_plain_date_equals_admission import (
    ...
    TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FILES,
...
_PLAIN_MONTH_DAY_TO_JSON_OUTSIDE_CANDIDATES = frozenset(
    {
        "built-ins/Temporal/PlainMonthDay/from/basic.js",
        (
            "intl402/DateTimeFormat/prototype/formatRangeToParts/"
            "temporal-objects-not-overlapping-options.js"
        ),
        (
            "intl402/DateTimeFormat/prototype/formatToParts/"
            "temporal-objects-not-overlapping-options.js"
        ),
        (
            "intl402/Temporal/PlainMonthDay/prototype/toLocaleString/"
            "lone-options-accepted.js"
        ),
    }
)


def _plain_month_day_to_json_candidate_audit(test_root):
    roots = (
        test_root / "built-ins/Temporal/PlainMonthDay",
        test_root / "intl402/Temporal/PlainMonthDay",
    ...
    for root in roots:
        for path in root.rglob("*.js"):
            ...
            if "PlainMonthDay" in source and (
                "toJSON" in source or "JSON.stringify" in source
            ):
                candidates.add(path.relative_to(test_root).as_posix())
    return frozenset(candidates)


def _plain_month_day_to_plain_date_candidate_audit(test_root):
    candidates = set()
    for path in test_root.rglob("*.js"):
        source = path.read_text()
        if re.search(r"\btoPlainDate\b", source) and re.search(
...
def _plain_month_day_helper_calls_to_plain_date(test262_root):
...
class ModuleCoreAdmissionTests(unittest.TestCase):
    ...
    def test_temporal_zoned_date_time_fixed_offset_manifest_is_exact_live_disjoint_and_shared(self):
        ...
        temporal_helpers = {
            "built-ins/Temporal/ZonedDateTime/from/argument-object.js",
            ...
            "built-ins/Temporal/ZonedDateTime/from/argument-string.js",
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                if path.name in {"length.js", "name.js", "prop-desc.js"}:
                    ...
                    includes = ["isConstructor.js"]
                elif path.name in {"preserves-instant.js", "subclassing-ignored.js"}:
                    includes = ["temporalHelpers.js"]
                else:
                    includes = []
                self.assertEqual(metadata.get("includes", []), includes, relative)
    ...
    def test_temporal_zoned_date_time_start_of_day_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_ZONED_DATE_TIME_START_OF_DAY_FILES
        features_by_file = TEMPORAL_ZONED_DATE_TIME_START_OF_DAY_FEATURES
        self.assertEqual(len(files), 9)
        self.assertEqual(set(features_by_file), set(files))

        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/ZonedDateTime/prototype/startOfDay"
        try:
            live_files = (
                {
                    path.relative_to(test_root).as_posix()
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(
                        tool.temporal_zoned_date_time_start_of_day_path(path),
                        relative,
                    )
                    self.assertEqual(
                        tool.temporal_zoned_date_time_start_of_day_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_zoned_date_time_start_of_day_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/ZonedDateTime/prototype/startOfDay/future.js"
            outside = root / "test/built-ins/Other/prototype/startOfDay/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
    ...
    def test_temporal_zoned_date_time_to_plain_date_time_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FILES
        features_by_file = TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FEATURES
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                if path.name in {"length.js", "name.js", "prop-desc.js"}:
                    ...
                    includes = ["isConstructor.js"]
                elif path.name in temporal_helpers:
                    includes = ["temporalHelpers.js"]
                else:
                    includes = []
                self.assertEqual(metadata.get("includes", []), includes, relative)
                ...
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(
                        tool.temporal_zoned_date_time_to_plain_date_time_path(path),
                        relative,
                    )
                    self.assertEqual(
                        tool.temporal_zoned_date_time_to_plain_date_time_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            future = (
                root
                / "test/built-ins/Temporal/ZonedDateTime/prototype/toPlainDateTime/future.js"
            )
            outside = (
                root
                / "test/built-ins/Other/prototype/toPlainDateTime/basic.js"
            )
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
    ...
    def test_temporal_duration_core_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_DURATION_CORE_FILES
        features_by_file = TEMPORAL_DURATION_CORE_FEATURES
        blockers = {
            line
            for raw_line in Path(__file__).with_name(
                "test262_temporal_duration_core_blockers.txt"
            ).read_text().splitlines()
            if (line := raw_line.strip()) and not line.startswith("#")
        }
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        duration_dir = test_root / "built-ins/Temporal/Duration"
        accessor_names = (
            "years",
            "months",
            "weeks",
        ...
        try:
            ...
            if live_files is not None:
                live_files.update(
                    path.relative_to(test_root).as_posix()
                    for path in (duration_dir / "prototype").glob("*.js")
                    if "_FIXTURE" not in path.name
                )
                for name in accessor_names:
                    live_files.update(
                        path.relative_to(test_root).as_posix()
                        for path in (duration_dir / "prototype" / name).glob("*.js")
                        if "_FIXTURE" not in path.name
                    )
        except OSError:
            live_files = None
        if live_files is not None:
            ...
            for relative in files:
                ...
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_duration_core_path(path), relative)
                    self.assertEqual(
                        tool.temporal_duration_core_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
            for relative in blockers:
        ...
        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_duration_core_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/Duration/prototype/sign/future.js"
            outside = root / "test/built-ins/Other/prototype/sign/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in features_by_file.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_duration_core_path(path), relative)
                        self.assertEqual(tool.temporal_duration_core_features(path), features)
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
    ...
    def test_temporal_duration_from_manifest_is_exact_live_disjoint_and_shared(self):
        ...
        self.assertEqual((len(admitted), len(blockers), len(all_files)), (31, 0, 31))
        self.assertEqual(blockers, frozenset())
    ...
    def test_temporal_duration_with_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_DURATION_WITH_FILES
        self.assertEqual(len(files), 22)
        self.assertEqual(set(TEMPORAL_DURATION_WITH_FEATURES), files)
        self.assertEqual(set(TEMPORAL_DURATION_WITH_INCLUDES), files)
        self.assertEqual(set(TEMPORAL_DURATION_WITH_FLAGS), files)
        self.assertEqual(set(TEMPORAL_DURATION_WITH_NEGATIVE), files)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_duration_with_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/Duration/prototype/with"
        corpus_required = "TEST262" in os.environ
        ...
        if live is not None:
            ...
            for relative in files:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_DURATION_WITH_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_DURATION_WITH_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_DURATION_WITH_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_DURATION_WITH_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_duration_with_path(path), relative)
                    self.assertEqual(
                        tool.temporal_duration_with_features(path),
                        TEMPORAL_DURATION_WITH_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/Duration/prototype/with/future.js"
            malformed = root / "test/built-ins/Temporal/Duration/prototype/with/basic.js.bak"
            outside = root / "test/built-ins/Temporal/Duration/with/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    ...
                    for path in (future, malformed, outside, None, object()):
                        self.assertFalse(tool.temporal_duration_with_path(path))
                        self.assertEqual(
                            tool.temporal_duration_with_features(path), frozenset()
                        )
                    for path in (future, malformed, outside):
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                    with patch("pathlib.Path.resolve", side_effect=PermissionError):
    ...
    def test_temporal_duration_to_string_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_DURATION_TO_STRING_FILES
        self.assertEqual(len(files), 44)
        self.assertEqual(set(TEMPORAL_DURATION_TO_STRING_FEATURES), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_STRING_INCLUDES), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_STRING_FLAGS), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_STRING_NEGATIVE), files)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_duration_to_string_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/Duration/prototype/toString"
        corpus_required = "TEST262" in os.environ
        ...
        if live is not None:
            ...
            for relative in files:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_DURATION_TO_STRING_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_DURATION_TO_STRING_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_DURATION_TO_STRING_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_DURATION_TO_STRING_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_duration_to_string_path(path), relative)
                    self.assertEqual(
                        tool.temporal_duration_to_string_features(path),
                        TEMPORAL_DURATION_TO_STRING_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/Duration/prototype/toString/future.js"
            malformed = future.with_name("basic.js.bak")
            outside = root / "test/built-ins/Temporal/Duration/toString/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    ...
                    for path in (future, malformed, outside, None, object()):
                        self.assertFalse(tool.temporal_duration_to_string_path(path))
                        self.assertEqual(
                            tool.temporal_duration_to_string_features(path), frozenset()
                        )
                    for path in (future, malformed, outside):
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                    with patch("pathlib.Path.resolve", side_effect=PermissionError):
                        self.assertFalse(tool.temporal_duration_to_string_path(future))
                        self.assertEqual(
                            tool.temporal_duration_to_string_features(future), frozenset()
                        )
                finally:
                    tool.TEST262 = original_root

    def test_temporal_duration_to_json_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_DURATION_TO_JSON_FILES
        self.assertEqual(len(files), 12)
        self.assertEqual(set(TEMPORAL_DURATION_TO_JSON_FEATURES), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_JSON_INCLUDES), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_JSON_FLAGS), files)
        self.assertEqual(set(TEMPORAL_DURATION_TO_JSON_NEGATIVE), files)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_duration_to_json_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/Duration/prototype/toJSON"
        corpus_required = "TEST262" in os.environ
        ...
        if live is not None:
            ...
            for relative in files:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_DURATION_TO_JSON_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_DURATION_TO_JSON_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_DURATION_TO_JSON_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_DURATION_TO_JSON_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_duration_to_json_path(path), relative)
                    self.assertEqual(
                        tool.temporal_duration_to_json_features(path),
                        TEMPORAL_DURATION_TO_JSON_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/Duration/prototype/toJSON/future.js"
            malformed = future.with_name("basic.js.bak")
            outside = root / "test/built-ins/Temporal/Duration/toJSON/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    ...
                    for path in (future, malformed, outside, None, object()):
                        self.assertFalse(tool.temporal_duration_to_json_path(path))
                        self.assertEqual(
                            tool.temporal_duration_to_json_features(path), frozenset()
                        )
                    for path in (future, malformed, outside):
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                    with patch("pathlib.Path.resolve", side_effect=PermissionError):
                        self.assertFalse(tool.temporal_duration_to_json_path(future))
                        self.assertEqual(
                            tool.temporal_duration_to_json_features(future), frozenset()
                        )
                finally:
                    tool.TEST262 = original_root

    def test_temporal_duration_total_manifest_is_exact_live_disjoint_and_shared(self):
        admitted = TEMPORAL_DURATION_TOTAL_FILES
        false_positives = TEMPORAL_DURATION_TOTAL_FALSE_POSITIVES
        blockers = TEMPORAL_DURATION_TOTAL_BLOCKERS
        ...
        self.assertFalse(false_positives & blockers)
        self.assertEqual(admitted | false_positives | blockers, all_files)
        self.assertEqual(set(TEMPORAL_DURATION_TOTAL_FEATURES), admitted)
        self.assertEqual(set(TEMPORAL_DURATION_TOTAL_ALL_FEATURES), all_files)
        self.assertEqual(set(TEMPORAL_DURATION_TOTAL_ALL_INCLUDES), all_files)
        self.assertEqual(set(TEMPORAL_DURATION_TOTAL_ALL_FLAGS), all_files)
        self.assertEqual(set(TEMPORAL_DURATION_TOTAL_ALL_NEGATIVE), all_files)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_duration_total_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/Duration/prototype/total"
        corpus_required = "TEST262" in os.environ
        ...
        if live is not None:
            ...
            for relative in all_files:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_DURATION_TOTAL_ALL_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_DURATION_TOTAL_ALL_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_DURATION_TOTAL_ALL_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_DURATION_TOTAL_ALL_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    if relative in admitted:
                        self.assertTrue(tool.temporal_duration_total_path(path), relative)
                        self.assertEqual(
                            tool.temporal_duration_total_features(path),
                            TEMPORAL_DURATION_TOTAL_FEATURES[relative],
                        )
                        self.assertFalse(tool.should_skip(metadata, path), relative)
                    else:
                        self.assertFalse(tool.temporal_duration_total_path(path), relative)
                        self.assertEqual(
                            tool.temporal_duration_total_features(path), frozenset()
                        )
                        self.assertTrue(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/Duration/prototype/total/future.js"
            malformed = future.with_name("total.js.bak")
            outside = root / "test/built-ins/Temporal/Duration/total/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    ...
                    for path in (future, malformed, outside, None, object()):
                        self.assertFalse(tool.temporal_duration_total_path(path))
                        self.assertEqual(
                            tool.temporal_duration_total_features(path), frozenset()
                        )
                    for path in (future, malformed, outside):
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                    with patch("pathlib.Path.resolve", side_effect=PermissionError):
                        self.assertFalse(tool.temporal_duration_total_path(future))
                        self.assertEqual(
                            tool.temporal_duration_total_features(future), frozenset()
                        )
                finally:
                    tool.TEST262 = original_root

    def test_temporal_plain_date_time_compare_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_DATE_TIME_COMPARE_FILES
        features_by_file = TEMPORAL_PLAIN_DATE_TIME_COMPARE_FEATURES
        blockers = {
            line
            for raw_line in Path(__file__).with_name(
                "test262_temporal_plain_date_time_compare_blockers.txt"
            ).read_text().splitlines()
            if (line := raw_line.strip()) and not line.startswith("#")
        }
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/PlainDateTime/compare"
        try:
            live_files = (
                {
                    path.relative_to(test_root).as_posix()
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                self.assertIsNone(metadata.get("negative"), relative)
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_plain_date_time_compare_path(path), relative)
                    self.assertEqual(
                        tool.temporal_plain_date_time_compare_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
            for relative in blockers:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainDateTime/compare/future.js"
            ...
            for tool in (test262_runner, test262_analyze):
                ...
                try:
                    for relative, features in features_by_file.items():
                        ...
                        self.assertTrue(tool.temporal_plain_date_time_compare_path(path), relative)
                        self.assertEqual(
                            tool.temporal_plain_date_time_compare_features(path), features
                        )
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_date_time_compare_path(path))
                        self.assertEqual(
                            tool.temporal_plain_date_time_compare_features(path), frozenset()
                        )
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                finally:
                    tool.TEST262 = original_root

    def test_temporal_plain_date_time_equals_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_DATE_TIME_EQUALS_FILES
        features_by_file = TEMPORAL_PLAIN_DATE_TIME_EQUALS_FEATURES
        blockers = {
            line
            for raw_line in Path(__file__).with_name(
                "test262_temporal_plain_date_time_equals_blockers.txt"
            ).read_text().splitlines()
            if (line := raw_line.strip()) and not line.startswith("#")
        }
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/PlainDateTime/prototype/equals"
        try:
            live_files = (
                {
                    path.relative_to(test_root).as_posix()
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_plain_date_time_equals_path(path), relative)
                    self.assertEqual(
                        tool.temporal_plain_date_time_equals_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
            for relative in blockers:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainDateTime/prototype/equals/future.js"
            outside = root / "test/built-ins/Other/prototype/equals/basic.js"
            for tool in (test262_runner, test262_analyze):
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in features_by_file.items():
                        ...
                        self.assertTrue(tool.temporal_plain_date_time_equals_path(path), relative)
                        self.assertEqual(
                            tool.temporal_plain_date_time_equals_features(path), features
                        )
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_date_time_equals_path(path))
                        self.assertEqual(
    ...
    def test_temporal_plain_date_equals_manifests_are_exact_live_and_shared(self):
        cases = (
            (
                TEMPORAL_PLAIN_DATE_EQUALS_FILES,
                TEMPORAL_PLAIN_DATE_EQUALS_FEATURES,
            ...
            (
                ...
                TEMPORAL_PLAIN_DATE_EQUALS_INTL_BLOCKER_FLAGS,
                TEMPORAL_PLAIN_DATE_EQUALS_INTL_BLOCKER_NEGATIVE,
                "test262_temporal_plain_date_equals_intl_blockers.txt",
                "intl402/Temporal/PlainDate/prototype/equals",
                "intl402/Temporal/PlainDate/prototype/equals/future.js",
                1,
                5,
                "temporal_plain_date_equals_intl_path",
                "temporal_plain_date_equals_intl_features",
            ),
        )
        self.assertTrue(
            TEMPORAL_PLAIN_DATE_EQUALS_FILES.isdisjoint(
                TEMPORAL_PLAIN_DATE_COMPARE_FILES
            )
        )
        self.assertTrue(
            TEMPORAL_PLAIN_DATE_EQUALS_INTL_FILES.isdisjoint(
                TEMPORAL_PLAIN_DATE_COMPARE_INTL_FILES
            )
        )
        ...
        downstream = TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FILES
        self.assertEqual(len(downstream), 4)
        self.assertEqual(set(TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FEATURES), downstream)
        self.assertEqual(set(TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_INCLUDES), downstream)
        self.assertEqual(set(TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FLAGS), downstream)
        ...
        if downstream_is_live:
            for relative in downstream:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_DATE_EQUALS_DOWNSTREAM_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertFalse(tool.temporal_plain_date_equals_path(path), relative)
                    self.assertFalse(
    ...
    def test_temporal_calendar_siblings_to_string_manifest_is_exact_live_and_shared(self):
        files = TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES
        ...
        def live_candidate_files():
            live_files = set()
            for kind in ("PlainMonthDay", "PlainYearMonth"):
                directory = test_root / "built-ins/Temporal" / kind / "prototype/toString"
                if not directory.is_dir():
                    if corpus_required:
                        raise FileNotFoundError(directory)
                    return None
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                self.assertEqual(
                    frozenset(metadata.get("features", [])),
                    TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FEATURES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("includes", [])),
                    TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_INCLUDES[relative],
                    relative,
                )
                self.assertEqual(
                    frozenset(metadata.get("flags", [])),
                    TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FLAGS[relative],
                    relative,
                )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_calendar_siblings_to_string_path(path))
                    self.assertEqual(
                        tool.temporal_calendar_siblings_to_string_features(path),
                        TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_calendar_siblings_to_string_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainMonthDay/prototype/toString/future.js"
            outside = root / "test/built-ins/Other/prototype/toString/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_calendar_siblings_to_string_path(None))
                self.assertFalse(tool.temporal_calendar_siblings_to_string_path(object()))
                self.assertEqual(
                    tool.temporal_calendar_siblings_to_string_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FEATURES.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_calendar_siblings_to_string_path(path))
                        self.assertEqual(
                            tool.temporal_calendar_siblings_to_string_features(path), features
                        )
                        self.assertFalse(
                            tool.should_skip({"features": sorted(features)}, path)
                        )
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_calendar_siblings_to_string_path(path))
                        self.assertEqual(
                            tool.temporal_calendar_siblings_to_string_features(path),
                            frozenset(),
                        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_sibling_to_string_diagnostic.test262_runner.TEST262
            temporal_sibling_to_string_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(
                    temporal_sibling_to_string_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_sibling_to_string_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/missing.js"],
                        ...
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_sibling_to_string_diagnostic.verify_expected_results(invalid)
                with patch.object(
                    temporal_sibling_to_string_diagnostic.test262_runner,
                    "run_test",
                    return_value="fail",
                ...
                temporal_sibling_to_string_diagnostic.test262_runner.TEST262 = original_root

    def test_temporal_calendar_siblings_with_manifest_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_CALENDAR_SIBLINGS_WITH_FILES
        ...
        try:
            for kind in ("PlainMonthDay", "PlainYearMonth"):
                directory = test_root / "built-ins/Temporal" / kind / "prototype/with"
                if not directory.is_dir():
                    if corpus_required:
                        raise FileNotFoundError(directory)
                    live_files = None
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_CALENDAR_SIBLINGS_WITH_FEATURES[relative]),
                    ("includes", TEMPORAL_CALENDAR_SIBLINGS_WITH_INCLUDES[relative]),
                    ("flags", TEMPORAL_CALENDAR_SIBLINGS_WITH_FLAGS[relative]),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_CALENDAR_SIBLINGS_WITH_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_calendar_siblings_with_path(path))
                    self.assertEqual(
                        tool.temporal_calendar_siblings_with_features(path),
                        TEMPORAL_CALENDAR_SIBLINGS_WITH_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_calendar_siblings_with_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainMonthDay/prototype/with/future.js"
            outside = root / "test/built-ins/Other/prototype/with/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_calendar_siblings_with_path(None))
                self.assertFalse(tool.temporal_calendar_siblings_with_path(object()))
                self.assertEqual(
                    tool.temporal_calendar_siblings_with_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in TEMPORAL_CALENDAR_SIBLINGS_WITH_FEATURES.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_calendar_siblings_with_path(path))
                        self.assertEqual(
                            tool.temporal_calendar_siblings_with_features(path), features
                        )
                        self.assertFalse(
                            tool.should_skip({"features": sorted(features)}, path)
                        )
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_calendar_siblings_with_path(path))
                        self.assertEqual(
                            tool.temporal_calendar_siblings_with_features(path), frozenset()
                        )
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                finally:
                    tool.TEST262 = original_root
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_sibling_with_diagnostic.test262_runner.TEST262
            temporal_sibling_with_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(
                    temporal_sibling_with_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_sibling_with_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/missing.js"],
                        ...
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_sibling_with_diagnostic.verify_expected_results(invalid)
                with patch.object(
                    temporal_sibling_with_diagnostic.test262_runner,
                    "run_test",
                    return_value="fail",
                ...
                temporal_sibling_with_diagnostic.test262_runner.TEST262 = original_root

    def test_temporal_plain_year_month_equals_surface_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_FILES
        self.assertEqual(files, temporal_year_month_equals_diagnostic.SURFACE)
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = test_root / "built-ins/Temporal/PlainYearMonth/prototype/equals"
        corpus_required = "TEST262" in os.environ
        try:
            live_files = {
                path.relative_to(test_root).as_posix()
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_FEATURES[relative]),
                    ("includes", TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_INCLUDES[relative]),
                    ("flags", TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_FLAGS[relative]),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_plain_year_month_equals_path(path), relative)
                    self.assertEqual(
                        tool.temporal_plain_year_month_equals_features(path),
                        TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_plain_year_month_equals_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainYearMonth/prototype/equals/future.js"
            outside = root / "test/built-ins/Other/prototype/equals/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_year_month_equals_path(None))
                self.assertFalse(tool.temporal_plain_year_month_equals_path(object()))
                self.assertEqual(
                    tool.temporal_plain_year_month_equals_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in TEMPORAL_PLAIN_YEAR_MONTH_EQUALS_FEATURES.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_plain_year_month_equals_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_equals_features(path), features
                        )
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_year_month_equals_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_equals_features(path), frozenset()
                        )
                        self.assertTrue(tool.should_skip({"features": ["Temporal"]}, path))
                finally:
                    tool.TEST262 = original_root
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_year_month_equals_diagnostic.test262_runner.TEST262
            temporal_year_month_equals_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(
                    temporal_year_month_equals_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_year_month_equals_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/missing.js"],
                        ...
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_year_month_equals_diagnostic.verify_expected_results(invalid)
                with patch.object(
                    temporal_year_month_equals_diagnostic.test262_runner,
                    "run_test",
                    return_value="fail",
                ...
                temporal_year_month_equals_diagnostic.test262_runner.TEST262 = original_root

    def test_temporal_plain_year_month_compare_surface_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FILES
        self.assertEqual(files, temporal_year_month_compare_diagnostic.SURFACE)
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = test_root / "built-ins/Temporal/PlainYearMonth/compare"
        corpus_required = "TEST262" in os.environ
        try:
            live_files = (
                {
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FEATURES[relative]),
                    ("includes", TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INCLUDES[relative]),
                    ("flags", TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FLAGS[relative]),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_plain_year_month_compare_path(path), relative)
                    self.assertEqual(
                        tool.temporal_plain_year_month_compare_features(path),
                        TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_plain_year_month_compare_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainYearMonth/compare/future.js"
            outside = root / "test/built-ins/Temporal/Other/compare/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_year_month_compare_path(None))
                self.assertFalse(tool.temporal_plain_year_month_compare_path(object()))
                self.assertEqual(
                    tool.temporal_plain_year_month_compare_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in (
                        TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FEATURES.items()
                    ):
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_plain_year_month_compare_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_compare_features(path), features
                        )
                        self.assertFalse(
                            tool.should_skip({"features": sorted(features)}, path)
                        )
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_year_month_compare_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_compare_features(path),
                            frozenset(),
                        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_year_month_compare_diagnostic.test262_runner.TEST262
            temporal_year_month_compare_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(
                    temporal_year_month_compare_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_year_month_compare_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/compare.js"],
                    ):
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_year_month_compare_diagnostic.verify_expected_results(
                                invalid
                            )
                with patch.object(
                    temporal_year_month_compare_diagnostic.test262_runner,
    ...
    def test_temporal_plain_year_month_compare_intl_surface_is_exact_live_disjoint_and_shared(self):
        ...
        self.assertEqual(files, surface - blockers)
        self.assertEqual(blockers, temporal_year_month_compare_intl_diagnostic.BLOCKERS)
        self.assertEqual(surface, temporal_year_month_compare_intl_diagnostic.SURFACE)
        self.assertEqual(len(files), 1)
        self.assertEqual(len(blockers), 3)
        self.assertEqual(len(surface), 4)
        self.assertTrue(files.isdisjoint(blockers))
        self.assertTrue(surface.isdisjoint(TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_FILES))
        for metadata_map in (
            TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INTL_FEATURES,
            TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INTL_INCLUDES,
            TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INTL_FLAGS,
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = test_root / "intl402/Temporal/PlainYearMonth/compare"
        corpus_required = "TEST262" in os.environ
        try:
            live_files = (
                {
        ...
        if corpus_required and live_files is None:
            raise FileNotFoundError(directory)
        if live_files is not None:
            self.assertEqual(live_files, set(surface))
            for relative in surface:
                path = test_root / relative
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                ...
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INTL_SURFACE_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    if relative in files:
                        self.assertTrue(
                            tool.temporal_plain_year_month_compare_intl_path(path),
                            relative,
                        )
                        self.assertEqual(
                            tool.temporal_plain_year_month_compare_intl_features(path),
                            TEMPORAL_PLAIN_YEAR_MONTH_COMPARE_INTL_FEATURES[relative],
                        )
                        self.assertFalse(tool.should_skip(metadata, path), relative)
                    else:
                        self.assertFalse(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/intl402/Temporal/PlainYearMonth/compare/future.js"
            outside = root / "test/intl402/Temporal/Other/compare/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_year_month_compare_intl_path(None))
                self.assertFalse(tool.temporal_plain_year_month_compare_intl_path(object()))
                self.assertEqual(
                    tool.temporal_plain_year_month_compare_intl_features(None),
                    frozenset(),
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative in surface:
                        ...
                        if relative in files:
                            ...
                            self.assertEqual(
                                tool.temporal_plain_year_month_compare_intl_features(
                                    path
                                ),
                                features,
                            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_root = root / "test"
            test_root.mkdir()
            diagnostic_runner = temporal_year_month_compare_intl_diagnostic.test262_runner
            original_root = diagnostic_runner.TEST262
            diagnostic_runner.TEST262 = str(root)
            arguments = sorted(surface)

            def expected_result(path):
                relative = Path(path).resolve().relative_to(test_root.resolve()).as_posix()
                return "fail" if relative in blockers else "pass"
            ...
            try:
                with patch.object(
                    ...
                    temporal_year_month_compare_intl_diagnostic.verify_expected_results(
                        arguments
                    )
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["intl402/Temporal/Other/compare.js"],
                    ):
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_year_month_compare_intl_diagnostic.verify_expected_results(
                                invalid
    ...
    def test_temporal_plain_month_day_to_json_surface_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FILES
        self.assertEqual(files, temporal_month_day_to_json_diagnostic.SURFACE)
        self.assertEqual(len(files), 7)
        for metadata_map in (
            TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FEATURES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INCLUDES,
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = test_root / "built-ins/Temporal/PlainMonthDay/prototype/toJSON"
        corpus_required = "TEST262" in os.environ
        try:
            live_files = (
                {
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FEATURES[relative]),
                    ("includes", TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INCLUDES[relative]),
                    ("flags", TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FLAGS[relative]),
                ):
                    self.assertEqual(
                        frozenset(metadata.get(key, [])), expected, relative
                    )
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(
                        tool.temporal_plain_month_day_to_json_path(path), relative
                    )
                    self.assertEqual(
                        tool.temporal_plain_month_day_to_json_features(path),
                        TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_plain_month_day_to_json_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / (
                "test/built-ins/Temporal/PlainMonthDay/prototype/toJSON/future.js"
            )
            outside = root / "test/built-ins/Temporal/Other/prototype/toJSON/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_month_day_to_json_path(None))
                self.assertFalse(tool.temporal_plain_month_day_to_json_path(object()))
                self.assertEqual(
                    tool.temporal_plain_month_day_to_json_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in (
                        TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FEATURES.items()
                    ):
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_plain_month_day_to_json_path(path))
                        self.assertEqual(
                            tool.temporal_plain_month_day_to_json_features(path),
                            features,
                        )
                    ...
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_month_day_to_json_path(path))
                        self.assertEqual(
                            tool.temporal_plain_month_day_to_json_features(path),
                            frozenset(),
                        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            diagnostic_runner = temporal_month_day_to_json_diagnostic.test262_runner
            original_root = diagnostic_runner.TEST262
            diagnostic_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(diagnostic_runner, "run_test", return_value="pass"):
                    temporal_month_day_to_json_diagnostic.verify_expected_results(
                        arguments
                    )
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments
                        + ["built-ins/Temporal/Other/prototype/toJSON/basic.js"],
                    ):
                        with self.assertRaisesRegex(
                            RuntimeError, "exact frozen surface"
                        ):
    ...
    def test_temporal_plain_month_day_to_json_intl_and_downstream_are_exact(self):
        files = TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_FILES
        blockers = TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_BLOCKERS
        surface = TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_SURFACE
        self.assertFalse(files)
        self.assertEqual(blockers, surface)
        self.assertEqual(blockers, temporal_month_day_to_json_intl_diagnostic.BLOCKERS)
        self.assertEqual(surface, temporal_month_day_to_json_intl_diagnostic.SURFACE)
        self.assertEqual(len(surface), 2)
        self.assertTrue(surface.isdisjoint(TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FILES))
        for metadata_map in (
            TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_FEATURES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_INCLUDES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_INTL_FLAGS,
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = test_root / "intl402/Temporal/PlainMonthDay/prototype/toJSON"
        audit_roots = (
            test_root / "built-ins/Temporal/PlainMonthDay",
            test_root / "intl402/Temporal/PlainMonthDay",
            test_root / "intl402/DateTimeFormat/prototype/formatRangeToParts",
        ...
        corpus_required = "TEST262" in os.environ
        try:
            corpus_available = directory.is_dir() and all(
                root.is_dir() for root in audit_roots
            )
        except OSError:
            if corpus_required:
                raise
            corpus_available = False
        if corpus_required and not corpus_available:
            raise FileNotFoundError(audit_roots)
        if corpus_available:
            live_files = {
                path.relative_to(test_root).as_posix()
                for path in directory.glob("*.js")
            ...
            expected_candidates = (
                TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FILES
                | surface
                | _PLAIN_MONTH_DAY_TO_JSON_OUTSIDE_CANDIDATES
            )
            self.assertEqual(
                _plain_month_day_to_json_candidate_audit(test_root),
                expected_candidates,
            )
            for relative in surface:
                path = test_root / relative
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            diagnostic = temporal_month_day_to_json_intl_diagnostic
            ...
            try:
                with (
                    ...
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["intl402/Temporal/Other/toJSON.js"],
                    ):
                        with self.assertRaisesRegex(
                            RuntimeError, "exact frozen surface"
                        ):
    ...
    def test_temporal_plain_month_day_to_plain_date_surface_is_exact_live_disjoint_and_shared(self):
        files = TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FILES
        self.assertEqual(files, temporal_month_day_to_plain_date_diagnostic.SURFACE)
        self.assertEqual(len(files), 12)
        for metadata_map in (
            TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FEATURES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INCLUDES,
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = (
            test_root / "built-ins/Temporal/PlainMonthDay/prototype/toPlainDate"
        )
        corpus_required = "TEST262" in os.environ
        try:
            live_files = (
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FEATURES[relative]),
                    ("includes", TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INCLUDES[relative]),
                    ("flags", TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FLAGS[relative]),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(
                        tool.temporal_plain_month_day_to_plain_date_path(path), relative
                    )
                    self.assertEqual(
                        tool.temporal_plain_month_day_to_plain_date_features(path),
                        TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FEATURES[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / (
                "test/built-ins/Temporal/PlainMonthDay/prototype/toPlainDate/future.js"
            )
            outside = root / "test/built-ins/Temporal/Other/toPlainDate/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_month_day_to_plain_date_path(None))
                self.assertFalse(
                    tool.temporal_plain_month_day_to_plain_date_path(object())
                )
                self.assertEqual(
                    tool.temporal_plain_month_day_to_plain_date_features(None),
                    frozenset(),
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in (
                        TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FEATURES.items()
                    ):
                        path = root / "test" / relative
                        self.assertTrue(
                            tool.temporal_plain_month_day_to_plain_date_path(path)
                        )
                        self.assertEqual(
                            tool.temporal_plain_month_day_to_plain_date_features(path),
                            features,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            diagnostic_runner = temporal_month_day_to_plain_date_diagnostic.test262_runner
            original_root = diagnostic_runner.TEST262
            diagnostic_runner.TEST262 = str(root)
            arguments = sorted(files)
            try:
                with patch.object(diagnostic_runner, "run_test", return_value="pass"):
                    temporal_month_day_to_plain_date_diagnostic.verify_expected_results(
                        arguments
                    )
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments
                        + ["built-ins/Temporal/Other/prototype/toPlainDate/basic.js"],
                    ):
                        with self.assertRaisesRegex(
                            RuntimeError, "exact frozen surface"
                        ):
    ...
    def test_temporal_plain_month_day_to_plain_date_intl_and_downstream_are_exact(self):
        files = TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_FILES
        blockers = TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_BLOCKERS
        surface = TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_SURFACE
        self.assertFalse(files)
        self.assertEqual(blockers, surface)
        self.assertEqual(surface, temporal_month_day_to_plain_date_intl_diagnostic.SURFACE)
        self.assertEqual(len(surface), 1)
        self.assertTrue(surface.isdisjoint(TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FILES))
        for metadata_map in (
            TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_FEATURES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_INCLUDES,
            TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_FLAGS,
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        directory = (
            test_root / "intl402/Temporal/PlainMonthDay/prototype/toPlainDate"
        )
        audit_roots = (
            test_root / "built-ins/Temporal/PlainMonthDay",
            test_root / "intl402/Temporal/PlainMonthDay",
        ...
        if corpus_available:
            ...
            self.assertEqual(live_files, set(surface))
            self.assertEqual(
                _plain_month_day_to_plain_date_candidate_audit(test_root),
                TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FILES | surface,
            )
            self.assertFalse(
                _plain_month_day_helper_calls_to_plain_date(test_root.parent)
            )
            for relative in surface:
                path = test_root / relative
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    (
                        "features",
                        TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_FEATURES[relative],
                    ),
                    (
                        "includes",
                        TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_INCLUDES[relative],
                    ),
                    (
                        "flags",
                        TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_FLAGS[relative],
                    ),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_INTL_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    self.assertFalse(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            diagnostic = temporal_month_day_to_plain_date_intl_diagnostic
            original_root = diagnostic.test262_runner.TEST262
            diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(surface)
            try:
                with (
                    ...
                    for invalid in (
                        [],
                        arguments + arguments,
                        arguments + ["intl402/Temporal/Other/toPlainDate.js"],
                    ):
                        with self.assertRaisesRegex(
                            RuntimeError, "exact frozen surface"
                        ):
    ...
    def test_temporal_plain_year_month_arithmetic_surface_is_exact_live_and_shared(self):
        ...
        blockers = temporal_year_month_arithmetic_diagnostic.BLOCKERS
        surface = temporal_year_month_arithmetic_diagnostic.SURFACE
        self.assertEqual(blockers, TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_BLOCKERS)
        self.assertEqual(surface, TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_SURFACE)
        self.assertEqual(len(files), 73)
        self.assertEqual(len(blockers), 0)
        self.assertEqual(len(surface), 73)
        self.assertTrue(files.isdisjoint(blockers))
        for metadata_map in (
            TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_FEATURES,
            TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_INCLUDES,
        ...
        try:
            for method in ("add", "subtract"):
                directory = (
                    test_root
                    / "built-ins/Temporal/PlainYearMonth/prototype"
                    / method
                )
                if not directory.is_dir():
                    if corpus_required:
        ...
        if live_files is not None:
            ...
            for relative in surface:
                ...
                metadata = test262_runner.parse_meta(path.read_text())
                for key, expected in (
                    ("features", TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_SURFACE_FEATURES[relative]),
                    ("includes", TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_SURFACE_INCLUDES[relative]),
                    ("flags", TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_SURFACE_FLAGS[relative]),
                ):
                    self.assertEqual(frozenset(metadata.get(key, [])), expected, relative)
                self.assertEqual(
                    metadata.get("negative"),
                    TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_SURFACE_NEGATIVE[relative],
                    relative,
                )
                for tool in (test262_runner, test262_analyze):
                    if relative in files:
                        self.assertTrue(tool.temporal_plain_year_month_arithmetic_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_arithmetic_features(path),
                            TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_FEATURES[relative],
                        )
                        self.assertFalse(tool.should_skip(metadata, path), relative)
                    else:
                        self.assertFalse(tool.temporal_plain_year_month_arithmetic_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_arithmetic_features(path), frozenset()
                        )
                        self.assertTrue(tool.should_skip(metadata, path), relative)

        tools_dir = Path(__file__).resolve().parent
        for manifest in tools_dir.glob("test262_*_admission.txt"):
            if manifest.name == "test262_temporal_plain_year_month_arithmetic_admission.txt":
                continue
            existing = {
                line
                for raw_line in manifest.read_text().splitlines()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainYearMonth/prototype/add/future.js"
            outside = root / "test/built-ins/Other/prototype/add/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_year_month_arithmetic_path(None))
                self.assertFalse(tool.temporal_plain_year_month_arithmetic_path(object()))
                self.assertEqual(
                    tool.temporal_plain_year_month_arithmetic_features(None), frozenset()
                )
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_FEATURES.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_plain_year_month_arithmetic_path(path))
                        self.assertEqual(
                            tool.temporal_plain_year_month_arithmetic_features(path), features
                        )
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_year_month_arithmetic_path(path))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_year_month_arithmetic_diagnostic.test262_runner.TEST262
            temporal_year_month_arithmetic_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(surface)
            try:
                with patch.object(
                    temporal_year_month_arithmetic_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_year_month_arithmetic_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/missing.js"],
                        ...
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_year_month_arithmetic_diagnostic.verify_expected_results(invalid)
                with patch.object(
                    temporal_year_month_arithmetic_diagnostic.test262_runner,
                    "run_test",
                    return_value="fail",
                ...
                temporal_year_month_arithmetic_diagnostic.test262_runner.TEST262 = original_root

    def test_temporal_calendar_siblings_formatter_surface_is_exact(self):
        passing = temporal_sibling_formatter_diagnostic.PASSING
        surface = temporal_sibling_formatter_diagnostic.SURFACE
        with_transitions = temporal_sibling_formatter_diagnostic.WITH_TRANSITIONS
        arithmetic_transitions = temporal_sibling_formatter_diagnostic.ARITHMETIC_TRANSITIONS
        self.assertEqual(len(passing), 128)
        self.assertEqual(len(surface), 128)
        self.assertEqual(len(with_transitions), 13)
        self.assertEqual(len(arithmetic_transitions), 22)
        self.assertTrue(with_transitions <= TEMPORAL_CALENDAR_SIBLINGS_WITH_FILES)
        self.assertTrue(arithmetic_transitions <= TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_FILES)
        self.assertEqual(surface, passing)
        ...
        if temporal_root_available:
            ...
            for kind, helper in (
                ...
                ("PlainYearMonth", "assertPlainYearMonth"),
            ):
                for path in (temporal_root / kind).rglob("*.js"):
                    source = path.read_text()
                    if f"TemporalHelpers.{helper}" in source or (
                        "/prototype/toString/" not in path.as_posix()
                        and re.search(r"\.toString\(", source)
                    ):
                        downstream.add(path.relative_to(test_root).as_posix())
            self.assertEqual(
                downstream | set(TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES),
                surface,
            )
        elif corpus_required:
            raise FileNotFoundError(temporal_root)

        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "test").mkdir()
            original_root = temporal_sibling_formatter_diagnostic.test262_runner.TEST262
            temporal_sibling_formatter_diagnostic.test262_runner.TEST262 = str(root)
            arguments = sorted(surface)
            try:
                with patch.object(
                    temporal_sibling_formatter_diagnostic.test262_runner,
                    "run_test",
                    return_value="pass",
                ):
                    temporal_sibling_formatter_diagnostic.verify_expected_results(arguments)
                    for invalid in (
                        arguments[:-1],
                        arguments[:-1] + [arguments[0]],
                        arguments + ["built-ins/Temporal/Other/missing.js"],
                        ...
                        with self.assertRaisesRegex(RuntimeError, "exact frozen surface"):
                            temporal_sibling_formatter_diagnostic.verify_expected_results(invalid)
                with patch.object(
                    temporal_sibling_formatter_diagnostic.test262_runner,
                    "run_test",
                    return_value="fail",
                ...
                temporal_sibling_formatter_diagnostic.test262_runner.TEST262 = original_root
    ...
    def test_temporal_plain_date_from_manifest_is_exact_live_disjoint_and_shared(self):
        ...
        includes_by_file = TEMPORAL_PLAIN_DATE_FROM_INCLUDES
        flags_by_file = TEMPORAL_PLAIN_DATE_FROM_FLAGS
        negative_by_file = TEMPORAL_PLAIN_DATE_FROM_NEGATIVE
        blockers = {
            line
            for raw_line in Path(__file__).with_name(
        ...
        test_root = Path(test262_runner.TEST262) / "test"
        method_dir = test_root / "built-ins/Temporal/PlainDate/from"
        try:
            live_files = (
                {
                    path.relative_to(test_root).as_posix()
        ...
        if live_files is not None:
            ...
            for relative in files:
                ...
                self.assertEqual(metadata.get("negative"), negative_by_file[relative], relative)
                for tool in (test262_runner, test262_analyze):
                    self.assertTrue(tool.temporal_plain_date_from_path(path), relative)
                    self.assertEqual(
                        tool.temporal_plain_date_from_features(path),
                        features_by_file[relative],
                    )
                    self.assertFalse(tool.should_skip(metadata, path), relative)
            for relative in blockers:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            future = root / "test/built-ins/Temporal/PlainDate/from/future.js"
            outside = root / "test/built-ins/Other/from/basic.js"
            for tool in (test262_runner, test262_analyze):
                self.assertFalse(tool.temporal_plain_date_from_path(None))
                self.assertFalse(tool.temporal_plain_date_from_path(object()))
                self.assertEqual(tool.temporal_plain_date_from_features(None), frozenset())
                original_root = tool.TEST262
                tool.TEST262 = str(root)
                try:
                    for relative, features in features_by_file.items():
                        path = root / "test" / relative
                        self.assertTrue(tool.temporal_plain_date_from_path(path), relative)
                        self.assertEqual(tool.temporal_plain_date_from_features(path), features)
                        self.assertFalse(tool.should_skip({"features": sorted(features)}, path))
                    for path in (future, outside):
                        self.assertFalse(tool.temporal_plain_date_from_path(path))
                        self.assertEqual(
