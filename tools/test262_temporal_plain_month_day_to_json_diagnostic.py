#!/usr/bin/env python3
"""Force the exact PlainMonthDay.prototype.toJSON built-ins directory."""
...
SURFACE = TEMPORAL_PLAIN_MONTH_DAY_TO_JSON_FILES
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError(
            "PlainMonthDay.toJSON diagnostic requires the exact frozen surface"
        )
    actual = {path: test262_runner.run_test(test_root / path) for path in SURFACE}
    expected = {path: "pass" for path in SURFACE}
    if actual != expected:
        raise RuntimeError(f"PlainMonthDay.toJSON results drifted: {actual}")
