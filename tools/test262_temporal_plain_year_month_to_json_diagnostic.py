#!/usr/bin/env python3
"""Force the exact PlainYearMonth.prototype.toJSON directory through RuJa."""
...
SURFACE = TEMPORAL_PLAIN_YEAR_MONTH_TO_JSON_FILES
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError("PlainYearMonth.toJSON diagnostic requires the exact frozen surface")
    actual = {path: test262_runner.run_test(test_root / path) for path in SURFACE}
    expected = {path: "pass" for path in SURFACE}
    if actual != expected:
        raise RuntimeError(f"PlainYearMonth.toJSON results drifted: {actual}")
