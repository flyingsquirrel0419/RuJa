...
try:
    ...
    from test262_temporal_instant_compare_admission import (
        TEMPORAL_INSTANT_COMPARE_FEATURES, TEMPORAL_INSTANT_COMPARE_FILES,
    )
    ...
    from test262_temporal_zoned_date_time_to_plain_date_time_admission import (
        TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FEATURES,
        TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FILES,
    )
    ...
    from test262_temporal_duration_with_admission import (
    ...
    from test262_temporal_duration_to_json_admission import (
        TEMPORAL_DURATION_TO_JSON_FEATURES,
        TEMPORAL_DURATION_TO_JSON_FILES,
    )
    ...
    from test262_temporal_calendar_siblings_with_admission import (
    ...
    from test262_temporal_plain_date_equals_admission import (
        TEMPORAL_PLAIN_DATE_EQUALS_FEATURES,
        TEMPORAL_PLAIN_DATE_EQUALS_FILES,
    )
    from test262_temporal_plain_date_equals_intl_admission import (
    ...
    from test262_temporal_plain_date_time_with_admission import (
    ...
    from test262_temporal_plain_date_time_equals_admission import (
    ...
    from tools.test262_temporal_instant_compare_admission import (
        TEMPORAL_INSTANT_COMPARE_FEATURES, TEMPORAL_INSTANT_COMPARE_FILES,
    )
    ...
    from tools.test262_temporal_instant_to_string_admission import (
        TEMPORAL_INSTANT_TO_STRING_FEATURES,
        TEMPORAL_INSTANT_TO_STRING_FILES,
    )
    ...
    from tools.test262_temporal_zoned_date_time_to_plain_date_time_admission import (
        TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FEATURES,
        TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FILES,
    )
    ...
    from tools.test262_temporal_duration_from_admission import (
        TEMPORAL_DURATION_FROM_FEATURES,
        TEMPORAL_DURATION_FROM_FILES,
    )
    ...
    from tools.test262_temporal_duration_value_of_admission import (
        TEMPORAL_DURATION_VALUE_OF_FEATURES,
        TEMPORAL_DURATION_VALUE_OF_FILES,
    )
    from tools.test262_temporal_duration_to_string_admission import (
        TEMPORAL_DURATION_TO_STRING_FEATURES,
        TEMPORAL_DURATION_TO_STRING_FILES,
    )
    from tools.test262_temporal_duration_to_json_admission import (
        TEMPORAL_DURATION_TO_JSON_FEATURES,
        TEMPORAL_DURATION_TO_JSON_FILES,
    )
    ...
    from tools.test262_temporal_calendar_siblings_from_admission import (
        TEMPORAL_CALENDAR_SIBLINGS_FROM_FEATURES,
        TEMPORAL_CALENDAR_SIBLINGS_FROM_FILES,
    )
    from tools.test262_temporal_calendar_siblings_to_string_admission import (
        TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FEATURES,
        TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES,
    )
    ...
    from tools.test262_temporal_plain_year_month_to_json_admission import (
        TEMPORAL_PLAIN_YEAR_MONTH_TO_JSON_FEATURES,
        TEMPORAL_PLAIN_YEAR_MONTH_TO_JSON_FILES,
    )
    from tools.test262_temporal_plain_year_month_to_plain_date_admission import (
        TEMPORAL_PLAIN_YEAR_MONTH_TO_PLAIN_DATE_FEATURES,
        TEMPORAL_PLAIN_YEAR_MONTH_TO_PLAIN_DATE_FILES,
    )
    ...
    from tools.test262_temporal_plain_date_equals_admission import (
        TEMPORAL_PLAIN_DATE_EQUALS_FEATURES,
        TEMPORAL_PLAIN_DATE_EQUALS_FILES,
    )
    from tools.test262_temporal_plain_date_equals_intl_admission import (
        TEMPORAL_PLAIN_DATE_EQUALS_INTL_FEATURES,
        TEMPORAL_PLAIN_DATE_EQUALS_INTL_FILES,
    )
    from tools.test262_temporal_plain_date_to_string_admission import (
        TEMPORAL_PLAIN_DATE_TO_STRING_FEATURES,
        TEMPORAL_PLAIN_DATE_TO_STRING_FILES,
    )
    ...
    from tools.test262_temporal_plain_date_time_conversions_admission import (
        TEMPORAL_PLAIN_DATE_TIME_CONVERSION_FEATURES,
        TEMPORAL_PLAIN_DATE_TIME_CONVERSION_FILES,
    )
    ...
    from tools.test262_temporal_plain_date_time_with_admission import (
        TEMPORAL_PLAIN_DATE_TIME_WITH_FEATURES,
        TEMPORAL_PLAIN_DATE_TIME_WITH_FILES,
    )
    from tools.test262_temporal_plain_date_time_difference_admission import (
        TEMPORAL_PLAIN_DATE_TIME_DIFFERENCE_FEATURES,
        TEMPORAL_PLAIN_DATE_TIME_DIFFERENCE_FILES,
    )
    ...
    from tools.test262_temporal_plain_date_time_from_admission import (
        TEMPORAL_PLAIN_DATE_TIME_FROM_FEATURES,
        TEMPORAL_PLAIN_DATE_TIME_FROM_FILES,
    )
...
def temporal_instant_core_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_INSTANT_CORE_FILES

def temporal_instant_core_features(path):
    if not temporal_instant_core_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_INSTANT_CORE_FEATURES[rel.as_posix()]

def temporal_instant_compare_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_INSTANT_COMPARE_FILES

def temporal_instant_compare_features(path):
    if not temporal_instant_compare_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_INSTANT_COMPARE_FEATURES[rel.as_posix()]
...
def temporal_instant_equals_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_INSTANT_EQUALS_FILES

def temporal_instant_equals_features(path):
    if not temporal_instant_equals_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_INSTANT_EQUALS_FEATURES[rel.as_posix()]

def temporal_instant_value_of_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_INSTANT_VALUE_OF_FILES

def temporal_instant_value_of_features(path):
    if not temporal_instant_value_of_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_zoned_date_time_fixed_offset_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_ZONED_DATE_TIME_FIXED_OFFSET_FILES

def temporal_zoned_date_time_fixed_offset_features(path):
    if not temporal_zoned_date_time_fixed_offset_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_zoned_date_time_with_time_zone_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in TEMPORAL_ZONED_DATE_TIME_WITH_TIME_ZONE_FILES

def temporal_zoned_date_time_with_time_zone_features(path):
    if not temporal_zoned_date_time_with_time_zone_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_ZONED_DATE_TIME_WITH_TIME_ZONE_FEATURES[rel.as_posix()]

def temporal_zoned_date_time_with_calendar_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_ZONED_DATE_TIME_WITH_CALENDAR_FILES

def temporal_zoned_date_time_with_calendar_features(path):
    if not temporal_zoned_date_time_with_calendar_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_zoned_date_time_to_plain_date_time_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FILES

def temporal_zoned_date_time_to_plain_date_time_features(path):
    if not temporal_zoned_date_time_to_plain_date_time_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_ZONED_DATE_TIME_TO_PLAIN_DATE_TIME_FEATURES[rel.as_posix()]

def temporal_duration_core_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_CORE_FILES

def temporal_duration_core_features(path):
    if not temporal_duration_core_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_CORE_FEATURES[rel.as_posix()]

def temporal_duration_from_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_FROM_FILES

def temporal_duration_from_features(path):
    if not temporal_duration_from_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_duration_with_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_WITH_FILES

def temporal_duration_with_features(path):
    if not temporal_duration_with_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_WITH_FEATURES[rel.as_posix()]

def temporal_duration_value_of_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_VALUE_OF_FILES

def temporal_duration_value_of_features(path):
    if not temporal_duration_value_of_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_VALUE_OF_FEATURES[rel.as_posix()]

def temporal_duration_to_string_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_TO_STRING_FILES

def temporal_duration_to_string_features(path):
    if not temporal_duration_to_string_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_TO_STRING_FEATURES[rel.as_posix()]

def temporal_duration_to_json_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_TO_JSON_FILES

def temporal_duration_to_json_features(path):
    if not temporal_duration_to_json_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_TO_JSON_FEATURES[rel.as_posix()]

def temporal_duration_total_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_DURATION_TOTAL_FILES

def temporal_duration_total_features(path):
    if not temporal_duration_total_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_DURATION_TOTAL_FEATURES[rel.as_posix()]
...
def _temporal_plain_time_features(path, files, features):
    if not _temporal_plain_time_path(path, files):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return features[rel.as_posix()]

def temporal_plain_time_core_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_CORE_FILES)

def temporal_plain_time_core_features(path):
    return _temporal_plain_time_features(
        path, TEMPORAL_PLAIN_TIME_CORE_FILES, TEMPORAL_PLAIN_TIME_CORE_FEATURES
    )

def temporal_plain_time_from_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_FROM_FILES)

def temporal_plain_time_from_features(path):
    return _temporal_plain_time_features(
        path, TEMPORAL_PLAIN_TIME_FROM_FILES, TEMPORAL_PLAIN_TIME_FROM_FEATURES
    )

def temporal_plain_time_value_of_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_VALUE_OF_FILES)

def temporal_plain_time_value_of_features(path):
    return _temporal_plain_time_features(
        path, TEMPORAL_PLAIN_TIME_VALUE_OF_FILES, TEMPORAL_PLAIN_TIME_VALUE_OF_FEATURES
    )

def temporal_plain_time_equals_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_EQUALS_FILES)

def temporal_plain_time_equals_features(path):
    return _temporal_plain_time_features(
...
def temporal_plain_time_to_string_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_TO_STRING_FILES)

def temporal_plain_time_to_string_features(path):
    return _temporal_plain_time_features(
        path,
        TEMPORAL_PLAIN_TIME_TO_STRING_FILES,
        TEMPORAL_PLAIN_TIME_TO_STRING_FEATURES,
...
def temporal_plain_time_to_json_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_TO_JSON_FILES)

def temporal_plain_time_to_json_features(path):
    return _temporal_plain_time_features(
        path,
        TEMPORAL_PLAIN_TIME_TO_JSON_FILES,
        TEMPORAL_PLAIN_TIME_TO_JSON_FEATURES,
...
def temporal_plain_time_round_path(path):
    return _temporal_plain_time_path(path, TEMPORAL_PLAIN_TIME_ROUND_FILES)

def temporal_plain_time_round_features(path):
    return _temporal_plain_time_features(
        path,
        TEMPORAL_PLAIN_TIME_ROUND_FILES,
        TEMPORAL_PLAIN_TIME_ROUND_FEATURES,
...
def temporal_calendar_siblings_from_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_CALENDAR_SIBLINGS_FROM_FILES

def temporal_calendar_siblings_from_features(path):
    if not temporal_calendar_siblings_from_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_CALENDAR_SIBLINGS_FROM_FEATURES[rel.as_posix()]

def temporal_calendar_siblings_to_string_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES

def temporal_calendar_siblings_to_string_features(path):
    if not temporal_calendar_siblings_to_string_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FEATURES[rel.as_posix()]

def temporal_calendar_siblings_with_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_CALENDAR_SIBLINGS_WITH_FILES

def temporal_calendar_siblings_with_features(path):
    if not temporal_calendar_siblings_with_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_CALENDAR_SIBLINGS_WITH_FEATURES[rel.as_posix()]

def temporal_plain_year_month_arithmetic_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_YEAR_MONTH_ARITHMETIC_FILES

def temporal_plain_year_month_arithmetic_features(path):
    if not temporal_plain_year_month_arithmetic_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_month_day_to_plain_date_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_MONTH_DAY_TO_PLAIN_DATE_FILES

def temporal_plain_month_day_to_plain_date_features(path):
    if not temporal_plain_month_day_to_plain_date_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_year_month_to_json_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_YEAR_MONTH_TO_JSON_FILES

def temporal_plain_year_month_to_json_features(path):
    if not temporal_plain_year_month_to_json_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_YEAR_MONTH_TO_JSON_FEATURES[rel.as_posix()]

def temporal_plain_year_month_to_plain_date_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_YEAR_MONTH_TO_PLAIN_DATE_FILES

def temporal_plain_year_month_to_plain_date_features(path):
    if not temporal_plain_year_month_to_plain_date_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_YEAR_MONTH_TO_PLAIN_DATE_FEATURES[rel.as_posix()]
...
def temporal_plain_date_from_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_FROM_FILES

def temporal_plain_date_from_features(path):
    if not temporal_plain_date_from_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_date_equals_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_EQUALS_FILES

def temporal_plain_date_equals_features(path):
    if not temporal_plain_date_equals_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_EQUALS_FEATURES[rel.as_posix()]

def temporal_plain_date_equals_intl_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_EQUALS_INTL_FILES

def temporal_plain_date_equals_intl_features(path):
    if not temporal_plain_date_equals_intl_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_EQUALS_INTL_FEATURES[rel.as_posix()]

def temporal_plain_date_to_string_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TO_STRING_FILES

def temporal_plain_date_to_string_features(path):
    if not temporal_plain_date_to_string_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TO_STRING_FEATURES[rel.as_posix()]

def temporal_plain_date_to_string_intl_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TO_STRING_INTL_FILES

def temporal_plain_date_to_string_intl_features(path):
    if not temporal_plain_date_to_string_intl_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_date_to_plain_date_time_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TO_PLAIN_DATE_TIME_FILES

def temporal_plain_date_to_plain_date_time_features(path):
    if not temporal_plain_date_to_plain_date_time_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_date_with_calendar_sibling_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_WITH_CALENDAR_SIBLING_FILES

def temporal_plain_date_with_calendar_sibling_features(path):
    if not temporal_plain_date_with_calendar_sibling_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_date_time_conversion_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_CONVERSION_FILES

def temporal_plain_date_time_conversion_features(path):
    if not temporal_plain_date_time_conversion_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TIME_CONVERSION_FEATURES[rel.as_posix()]

def temporal_plain_date_time_arithmetic_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_ARITHMETIC_FILES

def temporal_plain_date_time_arithmetic_features(path):
    if not temporal_plain_date_time_arithmetic_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_plain_date_time_with_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_WITH_FILES

def temporal_plain_date_time_with_features(path):
    if not temporal_plain_date_time_with_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TIME_WITH_FEATURES[rel.as_posix()]

def temporal_plain_date_time_difference_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_DIFFERENCE_FILES

def temporal_plain_date_time_difference_features(path):
    if not temporal_plain_date_time_difference_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TIME_DIFFERENCE_FEATURES[rel.as_posix()]
...
def temporal_plain_date_time_core_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_CORE_FILES

def temporal_plain_date_time_core_features(path):
    if not temporal_plain_date_time_core_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TIME_CORE_FEATURES[rel.as_posix()]

def temporal_plain_date_time_from_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_FROM_FILES

def temporal_plain_date_time_from_features(path):
    if not temporal_plain_date_time_from_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_PLAIN_DATE_TIME_FROM_FEATURES[rel.as_posix()]

def temporal_plain_date_time_equals_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_PLAIN_DATE_TIME_EQUALS_FILES

def temporal_plain_date_time_equals_features(path):
    if not temporal_plain_date_time_equals_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
...
def temporal_instant_to_string_path(path):
    if path is None:
        return False
    try:
        rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
        ...
        return False
    return rel.as_posix() in TEMPORAL_INSTANT_TO_STRING_FILES

def temporal_instant_to_string_features(path):
    if not temporal_instant_to_string_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to((Path(TEST262) / "test").resolve())
    return TEMPORAL_INSTANT_TO_STRING_FEATURES[rel.as_posix()]
...
def annex_b_escape_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in ANNEX_B_ESCAPE_FILES

def annex_b_escape_features(path):
    if not annex_b_escape_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to(Path(TEST262).resolve() / "test")
...
def regexp_compile_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in REGEXP_COMPILE_FILES

def regexp_compile_features(path):
    if not regexp_compile_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to(Path(TEST262).resolve() / "test")
...
def shadowrealm_path(path):
    ...
    try:
        ...
        return False
    return rel.as_posix() in SHADOWREALM_FILES

def shadowrealm_features(path):
    if not shadowrealm_path(path):
        return frozenset()
    rel = Path(path).resolve().relative_to(Path(TEST262).resolve() / "test")
...
def should_skip(meta, path=None):
    ...
    if path is not None and shadowrealm_path(path):
        feats.difference_update(shadowrealm_features(path))
    ...
    if path is not None and temporal_instant_compare_path(path):
        feats.difference_update(temporal_instant_compare_features(path))
    ...
    if path is not None and temporal_zoned_date_time_to_plain_date_time_path(path):
    ...
    if path is not None and temporal_duration_value_of_path(path):
        feats.difference_update(temporal_duration_value_of_features(path))
    if path is not None and temporal_duration_to_string_path(path):
        feats.difference_update(temporal_duration_to_string_features(path))
    if path is not None and temporal_duration_to_json_path(path):
        feats.difference_update(temporal_duration_to_json_features(path))
    if path is not None and temporal_duration_total_path(path):
        feats.difference_update(temporal_duration_total_features(path))
    ...
    if path is not None and temporal_plain_time_from_path(path):
        feats.difference_update(temporal_plain_time_from_features(path))
    if path is not None and temporal_plain_time_value_of_path(path):
    ...
    if path is not None and temporal_plain_time_to_string_path(path):
        feats.difference_update(temporal_plain_time_to_string_features(path))
    if path is not None and temporal_plain_time_to_json_path(path):
    ...
    if path is not None and temporal_calendar_siblings_from_path(path):
        feats.difference_update(temporal_calendar_siblings_from_features(path))
    if path is not None and temporal_calendar_siblings_to_string_path(path):
    ...
    if path is not None and temporal_plain_year_month_to_json_path(path):
        feats.difference_update(temporal_plain_year_month_to_json_features(path))
    if path is not None and temporal_plain_year_month_to_plain_date_path(path):
        feats.difference_update(temporal_plain_year_month_to_plain_date_features(path))
    ...
    if path is not None and temporal_plain_date_equals_path(path):
        feats.difference_update(temporal_plain_date_equals_features(path))
    if path is not None and temporal_plain_date_equals_intl_path(path):
        feats.difference_update(temporal_plain_date_equals_intl_features(path))
    if path is not None and temporal_plain_date_to_string_path(path):
    ...
    if path is not None and temporal_plain_date_with_calendar_sibling_path(path):
        feats.difference_update(
    ...
    if path is not None and temporal_plain_date_time_conversion_path(path):
    ...
    if path is not None and temporal_plain_date_time_with_path(path):
        feats.difference_update(temporal_plain_date_time_with_features(path))
    if path is not None and temporal_plain_date_time_difference_path(path):
        feats.difference_update(temporal_plain_date_time_difference_features(path))
    ...
    if path is not None and temporal_instant_to_string_path(path):
        feats.difference_update(temporal_instant_to_string_features(path))
