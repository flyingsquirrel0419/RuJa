"""Exact Test262 coverage for Temporal.PlainMonthDay.prototype.equals."""

from pathlib import Path


_MANIFEST = Path(__file__).with_name(
    "test262_temporal_plain_month_day_equals_admission.txt"
)
TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES = frozenset(
    line
...
TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FEATURES = {
    path: _features(path) for path in TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES
}
TEMPORAL_PLAIN_MONTH_DAY_EQUALS_INCLUDES = {
    path: _includes(path) for path in TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES
}
TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FLAGS = {
    path: frozenset() for path in TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES
}
TEMPORAL_PLAIN_MONTH_DAY_EQUALS_NEGATIVE = {
    path: None for path in TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES
}

if len(TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES) != 36:
    raise RuntimeError("PlainMonthDay.prototype.equals admission must contain 36 files")
