#!/usr/bin/env python3
"""Force the exact PlainMonthDay.prototype.equals directory through RuJa."""
...
SURFACE = TEMPORAL_PLAIN_MONTH_DAY_EQUALS_FILES
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError("PlainMonthDay equals diagnostic requires the exact frozen surface")
    actual = {path: test262_runner.run_test(test_root / path) for path in SURFACE}
    expected = {path: "pass" for path in SURFACE}
    if actual != expected:
        raise RuntimeError(f"PlainMonthDay equals results drifted: {actual}")
