#!/usr/bin/env python3
"""Force the exact Intl402 PlainMonthDay.toPlainDate companion."""
...
def _verify_corpus(test_root):
    for relative in SURFACE:
        ...
        if actual != expected:
            raise RuntimeError(
                f"PlainMonthDay.toPlainDate Intl metadata drifted: {relative}: {actual}"
            )
...
def verify_expected_results(arguments):
    ...
    if (
        ...
        raise RuntimeError(
            "PlainMonthDay.toPlainDate Intl diagnostic requires the exact frozen surface"
        )
    ...
    if actual != expected:
        raise RuntimeError(f"PlainMonthDay.toPlainDate Intl results drifted: {actual}")
    ...
    if wrong_errors:
        raise RuntimeError(
            f"PlainMonthDay.toPlainDate Intl failure reasons drifted: {wrong_errors}"
        )
