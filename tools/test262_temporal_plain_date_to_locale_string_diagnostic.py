#!/usr/bin/env python3
"""Force the frozen direct PlainDate.toLocaleString blocker surface."""
...
SURFACE = frozenset(
    line
    for raw_line in (
        TOOLS / "test262_temporal_plain_date_to_locale_string_blockers.txt"
...
EXPECTED_PASSES = frozenset(
    line
    for raw_line in (
        TOOLS / "test262_temporal_plain_date_to_locale_string_forced_passes.txt"
    ).read_text().splitlines()
    if (line := raw_line.strip()) and not line.startswith("#")
)
...
def verify_expected_results():
    ...
    if actual != expected:
        raise RuntimeError(f"direct PlainDate.toLocaleString forced results drifted: {actual}")
