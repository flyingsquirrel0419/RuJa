#!/usr/bin/env python3
"""Require the exact frozen partial-date toString surface to pass."""
...
def should_skip(meta, path=None):
    if path is not None and _relative(path) in TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES:
        return False
...
def verify_expected_results(arguments):
    ...
    if (
        len(arguments) != len(TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES)
        or None in requested
        or len(requested) != len(arguments)
        or requested != TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES
    ):
        raise RuntimeError(
            "Temporal calendar sibling toString diagnostic requires the exact frozen surface"
        )
    actual = {
        path: test262_runner.run_test(test_root / path)
        for path in TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES
    }
    expected = {
        path: "pass" for path in TEMPORAL_CALENDAR_SIBLINGS_TO_STRING_FILES
    }
    if actual != expected:
        raise RuntimeError(
            f"Temporal calendar sibling toString results drifted: {actual}"
        )
