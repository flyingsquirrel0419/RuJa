...
/// Check if a Value is a revoked proxy. Returns true if the value is a
/// Proxy and its revoked flag is set.
pub(crate) fn is_revoked_proxy(vm: &mut Vm, v: &Value) -> bool {
...
/// Get the (target, handler) from a proxy, or None if not a proxy.
pub(crate) fn get_proxy_target_handler(vm: &mut Vm, v: &Value) -> Option<(Value, Value)> {
    if let Value::Object(idx) = v {
        vm.heap.with_obj(idx.0, |o| {
            if let HeapObj::Proxy(p) = o {
                Some((p.target.clone(), p.handler.clone()))
            } else {
                None
            }
        })
...
/// Call a handler trap if it exists. Returns Ok(None) if the trap is not
/// defined (handler doesn't have the trap method).
pub(crate) fn call_trap(
    ...
    let trap = vm.get_property(handler, trap_name)?;
    if trap.is_undefined() {
        return Ok(None);
    }
    let result = vm.call_function(&trap, args, Some(handler.clone()))?;
    Ok(Some(result))
}
