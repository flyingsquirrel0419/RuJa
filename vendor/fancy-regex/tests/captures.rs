use fancy_regex::{Captures, CompileError, Error, Expander, Match, RegexBuilder, Result};
...
#[test]
fn ecmascript_backref_set_and_repeat_capture_clearing() {
    assert!(RegexBuilder::new(r"(?@1)").build().is_err());
    assert!(RegexBuilder::new(r"(?@0)")
        .ecmascript_mode(true)
        .build()
        .is_err());
    assert!(RegexBuilder::new(r"(?@0)")
        .ecmascript_mode(true)
        .ecmascript_backref_sets(vec![vec![1]])
        .build()
        .is_err());
    assert!(RegexBuilder::new(r"(?@0)")
        .ecmascript_mode(true)
        .ecmascript_backref_sets(vec![vec![]])
        .build()
        .is_err());
    assert!(RegexBuilder::new(r"(?@0)")
        .ecmascript_mode(true)
        .ecmascript_backref_sets(vec![vec![0]])
        .build()
        .is_err());
    for quantifier in ["{2}", "*", "+", "{1,3}?"] {
        let pattern = format!(r"^(?:(?:(a)|(b)|c)(?@0)){quantifier}$");
        let regex = RegexBuilder::new(&pattern)
            .ecmascript_mode(true)
            .ecmascript_backref_sets(vec![vec![1, 2]])
            .build()
            .expect("ECMAScript backend pattern");

        let captures = regex.captures("aabb").unwrap().expect("matched aliases");
        assert_match(captures.get(0), "aabb", 0, 4);
        assert!(captures.get(1).is_none());
        assert_match(captures.get(2), "b", 2, 3);

        let captures = regex
            .captures("aac")
            .unwrap()
            .expect("an unmatched alias uses the empty backreference");
        assert_match(captures.get(0), "aac", 0, 3);
        assert!(captures.get(1).is_none());
        assert!(captures.get(2).is_none());

        assert!(regex.captures("aaca").unwrap().is_none());
    }

    let unicode = RegexBuilder::new(r"^(?:(ſ)|(t))(?@0)$")
        ...
        .build()
        .unwrap();
    assert!(unicode.is_match("ſs").unwrap());

    let unicode_exact = RegexBuilder::new(r"^(?:(Ωa)|(z))(?@0)$")
        .case_insensitive(true)
        .ecmascript_mode(true)
        .ecmascript_unicode_mode(true)
        .ecmascript_backref_sets(vec![vec![1, 2]])
        .build()
        .unwrap();
    assert!(!unicode_exact.is_match("Ωaωaa").unwrap());

    let legacy = RegexBuilder::new(r"^(?:(ſ)|(t))(?@0)$")
        .case_insensitive(true)
        .ecmascript_mode(true)
        .ecmascript_unicode_mode(false)
        .ecmascript_backref_sets(vec![vec![1, 2]])
        .build()
        .unwrap();
    assert!(!legacy.is_match("ſs").unwrap());
}
