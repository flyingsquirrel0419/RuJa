    ...
    fn epsilon_closure_explore(
        ...
        loop {
            ...
            match *self.nfa.state(sid) {
                ...
                State::CaptureClear { next, slot } => {
                    for slot in [slot.as_usize(), slot.as_usize() + 1] {
                        if slot < curr_slots.len() {
                            let slot = SmallIndex::new(slot).unwrap();
                            stack.push(FollowEpsilon::RestoreCapture {
                                slot,
                                offset: curr_slots[slot],
                            });
                            curr_slots[slot] = None;
                        }
                    }
                    sid = next;
                }
...
mod ecmascript_tests {
    ...
    #[test]
    fn repeated_iterations_clear_descendant_captures() {
        let mut builder = PikeVM::builder();
        builder.thompson(thompson::Config::new().ecmascript_capture_clearing(true));
        let regex = builder.build(r"(?:(a)|(b))+").unwrap();
        let mut cache = regex.create_cache();
        let mut captures = regex.create_captures();
        regex.captures(&mut cache, "ab", &mut captures);

        assert!(captures.get_group(1).is_none());
        assert_eq!(captures.get_group(2).unwrap().range(), 1..2);
    }
}
