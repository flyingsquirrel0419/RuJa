...
pub struct Config {
    ...
    ecmascript_capture_clearing: Option<bool>,
    #[cfg(test)]
...
impl Config {
    ...
    /// Clear descendant capture slots before each repeated iteration.
    #[doc(hidden)]
    pub fn ecmascript_capture_clearing(mut self, yes: bool) -> Config {
        self.ecmascript_capture_clearing = Some(yes);
        self
    }
    ...
    pub(crate) fn overwrite(&self, o: Config) -> Config {
        Config {
            ...
            ecmascript_capture_clearing: o
                .ecmascript_capture_clearing
                .or(self.ecmascript_capture_clearing),
            #[cfg(test)]
            unanchored_prefix: o.unanchored_prefix.or(self.unanchored_prefix),
        }
    }

    fn get_ecmascript_capture_clearing(&self) -> bool {
        self.ecmascript_capture_clearing.unwrap_or(false)
    }
}
...
impl Compiler {
    ...
    fn c_repeat_iteration(&self, expr: &Hir) -> Result<ThompsonRef, BuildError> {
        let compiled = self.c(expr)?;
        if !self.config.get_ecmascript_capture_clearing()
            || !matches!(self.config.get_which_captures(), WhichCaptures::All)
        {
            return Ok(compiled);
        }

        fn collect(expr: &Hir, captures: &mut Vec<u32>) {
            match expr.kind() {
                hir::HirKind::Capture(capture) => {
                    captures.push(capture.index);
                    collect(&capture.sub, captures);
                }
                hir::HirKind::Repetition(repetition) => collect(&repetition.sub, captures),
                hir::HirKind::Concat(parts) | hir::HirKind::Alternation(parts) => {
                    for part in parts {
                        collect(part, captures);
                    }
                }
                _ => {}
            }
        ...
        collect(expr, &mut captures);
        captures.sort_unstable();
        captures.dedup();
        let mut start = compiled.start;
        for capture in captures.into_iter().rev() {
            let clear = self.add_capture_clear(capture)?;
            self.patch(clear, start)?;
            start = clear;
        }
        Ok(ThompsonRef {
            start,
    ...
    fn c_bounded(
        ...
        for _ in min..max {
            ...
            let compiled = self.c_repeat_iteration(expr)?;
    ...
    fn c_at_least(&self, expr: &Hir, greedy: bool, n: u32) -> Result<ThompsonRef, BuildError> {
        if n == 0 {
            ...
            let last = self.c_repeat_iteration(expr)?;
            let union = if greedy {
                self.add_union()
            } else {
                self.add_union_reverse()
    ...
    fn c_exactly(&self, expr: &Hir, n: u32) -> Result<ThompsonRef, BuildError> {
        let it = (0..n).map(|_| self.c_repeat_iteration(expr));
        self.c_concat(it)
    }
    ...
    fn c_look(&self, anchor: &hir::Look) -> Result<ThompsonRef, BuildError> {
        let look = match *anchor {
            ...
            hir::Look::WordEcmaUnicodeIgnoreCase => Look::WordEcmaUnicodeIgnoreCase,
            hir::Look::WordEcmaUnicodeIgnoreCaseNegate => Look::WordEcmaUnicodeIgnoreCaseNegate,
        };
    ...
    fn add_capture_clear(&self, capture_index: u32) -> Result<StateID, BuildError> {
        self.builder
            .borrow_mut()
            .add_capture_clear(StateID::ZERO, capture_index)
    }
