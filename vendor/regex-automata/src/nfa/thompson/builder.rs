...
enum State {
    ...
    /// An empty state that clears both slots for one capture group.
    CaptureClear {
        pattern_id: PatternID,
        group_index: SmallIndex,
        next: StateID,
...
impl Builder {
            ...
            match *state {
                ...
                State::CaptureClear {
                    ...
                    let slot = nfa
                        ...
                        .expect("invalid capture index");
                    let slot = SmallIndex::new(slot).expect("a small enough slot");
                    remap[sid] = nfa.add(nfa::State::CaptureClear { next, slot });
                }
    ...
    /// Add a state that clears both slots for one capture group.
    #[doc(hidden)]
    pub fn add_capture_clear(
        &mut self,
        next: StateID,
        group_index: u32,
    ) -> Result<StateID, BuildError> {
        let pattern_id = self.current_pattern_id();
        let group_index = SmallIndex::try_from(group_index)
            .map_err(|_| BuildError::invalid_capture_index(group_index))?;
        self.add(State::CaptureClear {
            pattern_id,
            group_index,
            next,
        })
    ...
    pub fn patch(&mut self, from: StateID, to: StateID) -> Result<(), BuildError> {
        ...
        match self.states[from] {
            ...
            State::CaptureClear { ref mut next, .. } => {
                *next = to;
            }
            State::Fail => {}
            State::Match { .. } => {}
