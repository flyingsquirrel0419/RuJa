...
pub enum Look {
    ...
    /// Match an ECMAScript Unicode ignore-case word boundary.
    WordEcmaUnicodeIgnoreCase = 1 << 18,
    /// Match a negated ECMAScript Unicode ignore-case word boundary.
    WordEcmaUnicodeIgnoreCaseNegate = 1 << 19,
}
...
impl Look {
    ...
    pub const fn reversed(self) -> Look {
        match self {
            ...
            Look::WordEcmaUnicodeIgnoreCase => Look::WordEcmaUnicodeIgnoreCase,
            Look::WordEcmaUnicodeIgnoreCaseNegate => Look::WordEcmaUnicodeIgnoreCaseNegate,
        }
    }
    ...
    pub const fn from_repr(repr: u32) -> Option<Look> {
        match repr {
            ...
            0x0004_0000 => Some(Look::WordEcmaUnicodeIgnoreCase),
            0x0008_0000 => Some(Look::WordEcmaUnicodeIgnoreCaseNegate),
            _ => None,
        }
    }
    ...
    pub const fn as_char(self) -> char {
        match self {
            ...
            Look::WordEcmaUnicodeIgnoreCase => 'e',
            Look::WordEcmaUnicodeIgnoreCaseNegate => 'E',
        }
    }
}
...
impl LookSet {
    ...
    pub fn contains_word_unicode(self) -> bool {
        self.contains(Look::WordUnicode)
            ...
            || self.contains(Look::WordEcmaUnicodeIgnoreCase)
            || self.contains(Look::WordEcmaUnicodeIgnoreCaseNegate)
    }
...
impl LookMatcher {
    ...
    pub(crate) fn matches_inline(&self, look: Look, haystack: &[u8], at: usize) -> bool {
        match look {
            ...
            Look::WordEcmaUnicodeIgnoreCase => self.is_word_ecma_unicode_ignore_case(haystack, at),
            Look::WordEcmaUnicodeIgnoreCaseNegate => {
                self.is_word_ecma_unicode_ignore_case_negate(haystack, at)
            }
    ...
    pub(crate) fn matches_set_inline(&self, set: LookSet, haystack: &[u8], at: usize) -> bool {
        ...
        if set.contains(Look::WordEcmaUnicodeIgnoreCase) {
            if !self.is_word_ecma_unicode_ignore_case(haystack, at) {
                return false;
            }
        }
        if set.contains(Look::WordEcmaUnicodeIgnoreCaseNegate) {
            if !self.is_word_ecma_unicode_ignore_case_negate(haystack, at) {
                return false;
            }
        }
    ...
    pub(crate) fn add_to_byteset(&self, look: Look, set: &mut crate::util::alphabet::ByteClassSet) {
        match look {
            ...
            Look::WordEcmaUnicodeIgnoreCase | Look::WordEcmaUnicodeIgnoreCaseNegate => {
    ...
    /// Returns true when the ECMAScript Unicode ignore-case word boundary is
    /// satisfied at `at`.
    #[inline]
    pub fn is_word_ecma_unicode_ignore_case(&self, haystack: &[u8], at: usize) -> bool {
        fn is_word(ch: char) -> bool {
            ch.is_ascii_alphanumeric() || ch == '_' || matches!(ch, '\u{017F}' | '\u{212A}')
        }

        let before = if at == 0 {
            false
        } else {
            let Some(Ok(ch)) = utf8::decode_last(&haystack[..at]) else {
                return false;
            };
            is_word(ch)
        };
        let after = if at == haystack.len() {
            false
        } else {
            let Some(Ok(ch)) = utf8::decode(&haystack[at..]) else {
                return false;
            };
            is_word(ch)
        };
    ...
    /// Returns true when the negated ECMAScript Unicode ignore-case word
    /// boundary is satisfied at `at`.
    #[inline]
    pub fn is_word_ecma_unicode_ignore_case_negate(&self, haystack: &[u8], at: usize) -> bool {
        if (at > 0 && utf8::decode_last(&haystack[..at]).is_none_or(|r| r.is_err()))
            || (at < haystack.len() && utf8::decode(&haystack[at..]).is_none_or(|r| r.is_err()))
        {
            return false;
        }
        !self.is_word_ecma_unicode_ignore_case(haystack, at)
    }
...
mod tests {
    ...
    #[test]
    fn look_matches_word_ecmascript_unicode_ignore_case() {
        let boundary = Look::WordEcmaUnicodeIgnoreCase;
        let non_boundary = Look::WordEcmaUnicodeIgnoreCaseNegate;

        assert!(testlook!(boundary, "a", 0));
        assert!(testlook!(boundary, "ſ", 0));
        assert!(testlook!(boundary, "K", 0));
        assert!(testlook!(boundary, "éa", 2));
        assert!(!testlook!(boundary, "é", 0));
        assert!(!testlook!(boundary, "中", 0));
        assert!(testlook!(non_boundary, "é", 0));
        assert!(!testlook!(non_boundary, "ſ", 0));
    }
