    ...
    if !state.look_need().is_empty() {
        ...
        if state.is_from_word() == unit.is_word_byte() {
            look_have = look_have
                ...
                .insert(Look::WordEcmaUnicodeIgnoreCaseNegate);
        } else {
            look_have = look_have
                .insert(Look::WordAscii)
                .insert(Look::WordUnicode)
                .insert(Look::WordEcmaUnicodeIgnoreCase);
        }
    ...
    while let Some(mut id) = stack.pop() {
        ...
        loop {
            ...
            match *nfa.state(id) {
                ...
                thompson::State::CaptureClear { next, .. } => {
                    id = next;
                }
            }
        }
...
pub(crate) fn add_nfa_states(nfa: &thompson::NFA, set: &SparseSet, builder: &mut StateBuilderNFA) {
    for nfa_id in set.iter() {
        match *nfa.state(nfa_id) {
            ...
            thompson::State::Capture { .. } | thompson::State::CaptureClear { .. } => {}
