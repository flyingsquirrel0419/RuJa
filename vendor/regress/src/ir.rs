...
pub enum Node {
    ...
    BackRef {
        group: u32,
        icase: bool,
        fail_if_unmatched: bool,
    },

    /// Matches empty only when every listed capture group did not participate.
    AllBackRefsUnmatched { groups: Vec<u32> },
...
impl Node {
    ...
    pub fn try_duplicate(&self, mut depth: usize) -> Option<Node> {
        ...
        Some(match self {
            ...
            Node::AllBackRefsUnmatched { groups } => Node::AllBackRefsUnmatched {
                groups: groups.clone(),
            },
...
fn display_node(node: &Node, depth: usize, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    match node {
        ...
        Node::AllBackRefsUnmatched { groups } => {
            writeln!(f, "AllBackRefsUnmatched {:?} ", groups)?;
        }
