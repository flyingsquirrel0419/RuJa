...
pub(crate) struct WorkLimitExceeded;

fn consume_work(work_remaining: &mut usize, amount: usize) -> Result<(), WorkLimitExceeded> {
    if *work_remaining < amount {
        *work_remaining = 0;
...
fn state_allocation_cost(re: &CompiledRegex) -> usize {
    1usize
        .saturating_add(re.groups as usize)
        .saturating_add(re.loops as usize)
}

fn state_clone_cost(re: &CompiledRegex) -> usize {
    64usize
        .saturating_add((re.groups as usize).saturating_mul(32))
        .saturating_add((re.loops as usize).saturating_mul(32))
}

fn group_names_cost(re: &CompiledRegex) -> usize {
    re.group_names.iter().fold(0usize, |cost, name| {
        cost.saturating_add(1).saturating_add(name.len())
    })
}

const MAX_BOUNDED_LIVE_STATE_BYTES: usize = 64 * 1024 * 1024;

fn continuation_requires_input_end(re: &CompiledRegex, continuation: usize) -> bool {
    matches!(
        re.insns.get(continuation),
        Some(Insn::EndOfLine { multiline: false })
    ) && matches!(re.insns.get(continuation + 1), Some(Insn::Goal))
}
...
fn run_loop<Position: PositionType>(
    ...
    clone_cost: usize,
) -> Result<StateMatch<Position>, WorkLimitExceeded> {
    ...
    Ok(if !enter_ok && !skip_ok {
        ...
        exit_state.ip = exit;
        consume_work(work_remaining, clone_cost)?;
...
fn try_match_state<Input: InputIndexer, Dir: Direction>(
    ...
    work_remaining: &mut usize,
    bounded: bool,
    live_state_bytes: &mut usize,
) -> Result<StateMatch<Input::Position>, WorkLimitExceeded> {
    consume_work(work_remaining, 1)?;
    ...
    Ok(match &re.insns[s.ip] {
        ...
        &Insn::Alt { secondary } => {
            consume_work(work_remaining, state_clone_cost(re))?;
        ...
        &Insn::BackRef {
            ...
            if let Some(orig_range) = group.as_range() {
                let units = input
                    .pos_to_offset(orig_range.end)
                    .saturating_sub(input.pos_to_offset(orig_range.start));
                consume_work(work_remaining, units.max(1))?;
                if icase {
                ...
                // (ES6 21.2.2.9).
                matched = !fail_if_unmatched;
            }
            nextinsn_or_fail!(matched)
        }

        Insn::AllBackRefsUnmatched(groups) => nextinsn_or_fail!(
            groups
                .iter()
                .all(|group| s.groups[*group as usize].as_range().is_none())
        ),
        ...
        &Insn::Lookahead {
            ...
            s.ip += 1;
            let saved_pos = s.pos;
            let attempt_succeeded = MatchAttempter::<Input>::new(re).try_at_pos(
                *input,
                s,
                Forward::new(),
                work_remaining,
        ...
        &Insn::Lookbehind {
            ...
            s.ip += 1;
            let saved_pos = s.pos;
            let attempt_succeeded = MatchAttempter::new(re).try_at_pos(
                *input,
                s,
                Backward::new(),
                work_remaining,
        ...
        Insn::EnterLoop(lf) => run_loop(s, lf, true, work_remaining, state_clone_cost(re))?,
        &Insn::LoopAgain { begin } => {
            s.ip = begin as usize;
            match re.insns.iat(s.ip) {
                Insn::EnterLoop(lf) => {
                    run_loop(s, lf, false, work_remaining, state_clone_cost(re))?
                }
        ...
        &Insn::Loop1CharBody {
            ...
            if iters < max_iters {
                ...
                let saved_pos = s.pos;
                s.ip = loop_ip + 1;
                taken_pos = match try_match_state(
                    re,
                    input,
                    s,
                    dir,
            ...
            match (taken_pos, iters >= min_iters) {
                ...
                (Some(taken_pos), true) => {
                    if greedy && continuation_requires_input_end(re, continuation) {
                        s.pos = taken_pos;
                        s.loop1_iters = iters + 1;
                        return Ok(StateMatch::Continue);
                    }
                    consume_work(work_remaining, state_clone_cost(re))?;
                    if greedy {
...
impl<'a, Input: InputIndexer> MatchAttempter<'a, Input> {
    ...
    fn try_at_pos<Dir: Direction>(
        ...
        bounded: bool,
        live_state_bytes: &mut usize,
    ) -> Result<bool, WorkLimitExceeded> {
        debug_assert!(self.states.is_empty(), "Should be no states");
        let clone_cost = state_clone_cost(self.re);
        let initial_capacity_bytes = self.states.capacity().saturating_mul(clone_cost);
        if bounded {
            if live_state_bytes.saturating_add(initial_capacity_bytes)
                >= MAX_BOUNDED_LIVE_STATE_BYTES
            {
                return Err(WorkLimitExceeded);
            }
            *live_state_bytes = live_state_bytes.saturating_add(initial_capacity_bytes);
        }

        let result = (|| {
            reserve_state_slot(&mut self.states, clone_cost, bounded, live_state_bytes)?;
            consume_work(work_remaining, state_allocation_cost(self.re))?;
            self.states.push(init_state.clone());
            while !self.states.is_empty() {
                let s = self.states.last_mut().unwrap();
                match try_match_state(
                    self.re,
                    &input,
                    s,
                    dir,
        ...
        if bounded {
            *live_state_bytes =
                live_state_bytes.saturating_sub(self.states.capacity().saturating_mul(clone_cost));
        }
        self.states.clear();
        result
    }
...
fn reserve_state_slot<Position: PositionType>(
    states: &mut Vec<State<Position>>,
    clone_cost: usize,
    bounded: bool,
    live_state_bytes: &mut usize,
    ...
    let old_capacity = states.capacity();
    if bounded && live_state_bytes.saturating_add(clone_cost) >= MAX_BOUNDED_LIVE_STATE_BYTES {
        return Err(WorkLimitExceeded);
    }
    states.try_reserve_exact(1).map_err(|_| WorkLimitExceeded)?;
    if bounded {
        let added = states
            .capacity()
            .saturating_sub(old_capacity)
            .saturating_mul(clone_cost);
        *live_state_bytes = live_state_bytes.saturating_add(added);
        if *live_state_bytes >= MAX_BOUNDED_LIVE_STATE_BYTES {
            return Err(WorkLimitExceeded);
        }
    }
    Ok(())
        ...
        if matches!(re.start_pred, StartPredicate::StartAnchored) {
            ...
            if self
                ...
                .try_at_pos(
                    ...
                    &mut work_remaining,
                    false,
                    &mut live_state_bytes,
                )
                .expect("unbounded PikeVM execution cannot exhaust its work budget")
            {
                let end = state.pos;
                if end != pos {
                    *next_start = Some(end)
...
#[cfg(feature = "utf16")]
pub(crate) fn find_from_utf16_bounded(
    re: &CompiledRegex,
    text: &[u16],
    start: usize,
    work_remaining: &mut usize,
) -> Result<Option<Match>, WorkLimitExceeded> {
    let input = Utf16Input::new(text, re.flags.unicode || re.flags.unicode_sets);
    let Some(mut pos) = input.try_move_right(input.left_end(), start) else {
        return Ok(None);
    };
    let mut matcher = MatchAttempter::new(re);
    let mut live_state_bytes = 0;

    loop {
        consume_work(work_remaining, state_allocation_cost(re))?;
        let mut state = State {
            pos,
            ip: 0,
            loop1_iters: 0,
        ...
        if matcher.try_at_pos(
            ...
            consume_work(work_remaining, group_names_cost(re))?;
            return Ok(Some(successful_match(
                input,
                pos,
                &state,
                re.group_names.clone(),
        ...
        let Some(next) = input.next_right_pos(pos) else {
            return Ok(None);
        };
        pos = next;
    }
...
#[cfg(feature = "utf16")]
pub(crate) fn find_at_utf16_bounded(
    re: &CompiledRegex,
    text: &[u16],
    start: usize,
    work_remaining: &mut usize,
    ...
    let input = Utf16Input::new(text, re.flags.unicode || re.flags.unicode_sets);
    let Some(pos) = input.try_move_right(input.left_end(), start) else {
        return Ok(None);
    };
    consume_work(work_remaining, state_allocation_cost(re))?;
    let mut state = State {
