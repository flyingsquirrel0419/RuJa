...
const MAX_NAMED_CAPTURE_GROUPS: usize = 1_024;
const MAX_DUPLICATE_CAPTURE_GROUPS_PER_NAME: usize = 64;
    ...
    fn case_closed(self, icase: bool) -> Self {
        ...
        match self {
            Self::ClassSetCharacter(code_point) => {
                ...
                Self::CharacterClassEscape(unicode::add_icase_code_points(codepoints))
            }
            Self::CharacterClassEscape(codepoints) => {
                Self::CharacterClassEscape(unicode::add_icase_code_points(codepoints))
            }
...
fn add_class_set_range(result: &mut ClassSet, first: u32, last: u32, icase: bool) {
    let mut codepoints = CodePointSet::new();
    codepoints.add(Interval { first, last });
    if icase {
        codepoints = unicode::add_icase_code_points(codepoints);
    }
    result.codepoints.add_set(codepoints);
}
    ...
    while nodes.len() > 1 {
        ...
        while let Some(left) = pairs.next() {
            if let Some(right) = pairs.next() {
                next.push(ir::Node::Alt(Box::new(left), Box::new(right)));
            } else {
                next.push(left);
            }
        }
...
fn codepoints_from_v_class(ct: CharacterClassType, positive: bool, icase: bool) -> CodePointSet {
...
fn codepoints_from_v_property(intervals: &[Interval], positive: bool, icase: bool) -> CodePointSet {
    let mut cps = CodePointSet::from_sorted_disjoint_intervals(intervals.to_vec());
    if icase {
        cps = unicode::add_icase_code_points(cps);
    }
    if positive { cps } else { cps.inverted() }
    ...
    I: Iterator<Item = u32> + Clone,
{
                ...
                if self.peek() == Some(0x2D /* - */) {
                    ...
                    match first {
                        ClassSetOperand::ClassSetCharacter(first) => {
                            ...
                            add_class_set_range(&mut result, first, last, self.flags.icase);
                        }
                        _ => {
                            return error("Invalid class set range");
                        }
            ...
            ClassSetOperator::Union => {
                loop {
                    ...
                    if self.peek() == Some(0x2D /* - */) {
                        ...
                        result.union_operand(operand, self.flags.icase);
                    }
                }
            }
        ...
        match cp {
            ...
            0x5C /* \ */ => {
                ...
                match cp {
                    ...
                    0x71 /* q */ => {
                        if self.flags.disable_string_sets {
                            return error("String-valued UnicodeSets are disabled");
                        }
                        self.consume('q');
                        if !self.try_consume('{') {
                    ...
                    0x64 /* d */ => {
                        self.consume('d');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Digits, true, self.flags.icase,
                        )))
                    }
                    // CharacterClassEscape :: D
                    0x44 /* D */ => {
                        self.consume('D');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Digits, false, self.flags.icase,
                        )))
                    }
                    ...
                    0x73 /* s */ => {
                        self.consume('s');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Spaces, true, self.flags.icase,
                        )))
                    }
                    // CharacterClassEscape :: S
                    0x53 /* S */ => {
                        self.consume('S');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Spaces, false, self.flags.icase,
                        )))
                    }
                    ...
                    0x77 /* w */ => {
                        self.consume('w');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Words, true, self.flags.icase,
                        )))
                    }
                    // CharacterClassEscape :: W
                    0x57 /* W */ => {
                        self.consume('W');
                        Ok(CharacterClassEscape(codepoints_from_v_class(
                            CharacterClassType::Words, false, self.flags.icase,
                        )))
                    }
                    ...
                    0x70 /* p */ => {
                        ...
                        match self.try_consume_unicode_property_escape()? {
                            PropertyEscapeKind::CharacterClass(intervals) => {
                                Ok(CharacterClassEscape(codepoints_from_v_property(
                                    intervals,
                                    true,
                                    self.flags.icase,
                                )))
            ...
            'k' if self.flags.unicode || !self.named_group_indices.is_empty() => {
                ...
                let node = match group_indices.len() {
                    ...
                    _ => {
                        ...
                        self.duplicate_backref_expansion = self
                        ...
                        let mut alternatives: Vec<_> = group_indices
                            ...
                            .collect();
                        alternatives.push(ir::Node::AllBackRefsUnmatched {
                            groups: group_indices.iter().map(|group| group + 1).collect(),
                        });
                        make_alt(alternatives)
                    }
                };
                Ok(node)
            }
    ...
    fn validate_resource_limits(&mut self) -> Result<(), Error> {
        self.parse_capture_groups()?;

        let original_input = self.input.clone();
        let mut class_depth = 0usize;
        while let Some(ch) = self.next().map(to_char_sat) {
            match ch {
                '\\' if class_depth == 0 && self.peek() == Some('k' as u32) => {
                    self.consume('k');
                    if let Some(name) = self.try_consume_named_capture_group_name()
                        && let Some(groups) = self.named_group_indices.get(&name)
                        && groups.len() > 1
                    {
                        self.duplicate_backref_expansion = self
                            .duplicate_backref_expansion
                ...
                '[' => {
                    if class_depth == 0 || self.flags.unicode_sets {
                        class_depth += 1;
                    }
                }
...
/// Validate bounded named-capture preprocessing without building regex IR.
pub(crate) fn validate_resource_limits<I>(pattern: I, flags: api::Flags) -> Result<(), Error>
where
    I: Iterator<Item = u32> + Clone,
{
    ...
    parser.validate_resource_limits()
}
