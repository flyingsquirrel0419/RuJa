...
impl<'a> InputIndexer for Utf16Input<'a> {
    ...
    fn subrange_eq<Dir: Direction>(
        ...
        if self.unicode {
            let captured = Utf16Input::new(
                &self.input[self.pos_to_offset(range.start)..self.pos_to_offset(range.end)],
                true,
            );
            let mut captured_pos = if Dir::FORWARD {
                captured.left_end()
            } else {
                captured.right_end()
            };
            ...
            loop {
                let expected = if Dir::FORWARD {
                    captured.next_right(&mut captured_pos)
                } else {
                    captured.next_left(&mut captured_pos)
                };
                let Some(expected) = expected else {
                    *pos = candidate_pos;
                    return true;
                };
                let actual = if Dir::FORWARD {
                    self.next_right(&mut candidate_pos)
                } else {
                    self.next_left(&mut candidate_pos)
                };
