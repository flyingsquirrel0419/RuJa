...
and `pause`, Realm-local `%Intl%`, `Intl.getCanonicalLocales`, and the base
`Intl.Locale` constructor/accessor/likely-subtag surface, file-backed
...
**Supported-subset pass rate: 100.0%** (12,757 tests in `language/statements`
+ `language/expressions` on current Test262, unsupported-feature tests
skipped). The latest full matrix is 65.8% of all files and 86.2% of executed
files (31,890 pass / 5,115 fail / 11,459 skip / 3 timeout) — see
