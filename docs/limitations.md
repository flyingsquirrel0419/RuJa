...
- test262 conformance is scoped, not full: RuJa targets a deliberately
  ...
  rate of 60.9% of all matrix files and 81.9% of executed files; within the
...
- The global `Iterator` and common synchronous iterator prototype hierarchy
  ...
  `flatMap`/`take`/`drop` helpers and eager `reduce`/`forEach`/`some`.
  Remaining helpers (`every`, `find`, and related methods),
