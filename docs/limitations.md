...
- test262 conformance is scoped, not full: RuJa targets a deliberately
  ...
  rate of 60.6% of all matrix files and 81.9% of executed files; within the
...
- The global `Iterator` and common synchronous iterator prototype hierarchy
  ...
  `take`/`drop` helpers. Remaining helpers (`flatMap`, `reduce`, `forEach`,
  `some`, `every`, `find`, and related methods),
