...
byte-identical. Only `test262_built-ins_result.txt` changes from
**15652/4887/3124/5/0** to **15752/4793/3119/4/0**, exactly **+100 pass / -94
fail / -5 skip / -1 timeout**; error, total, and every unrelated shard remain
unchanged. The downloaded release binary independently passes direct Array
forEach **190/190** and TypedArray forEach **42/42** on fixed checkout
`9e61c12835c5e4a3bdba93850427e6742c4f64c4`.
...
CI `29825824540` and full matrix `29825824539` pass all jobs. Downloaded
artifacts aggregate to **31664 pass / 5321 fail / 11479 skip / 3 timeout / 0
error / 48467 total / 36985 run**. Against map run `29821941873`, this is
exactly **+171 pass / -166 fail / -5 skip**. The built-ins shard moves from
**15883/4676/3106/3/0** to **16054/4510/3101/3/0**; every other result file is
byte-identical. The downloaded release binary again reports Array reduce
**260/260** and TypedArray reduce **50/50**.
...
`tools/test262_array_reduce_right_admission.txt` freezes exactly **5** direct
`Array.prototype.reduceRight` files otherwise hidden by broad TypedArray,
resizable-buffer, `Reflect.construct`, or arrow-function gates. Its exact
feature map is shared by runner and analyzer. Tooling checks map/manifest
equality, live features and includes, empty flags, absent negative metadata,
disjointness, runner/analyzer symmetry, extra-feature rejection, and closure
...
On fixed checkout `9e61c12835c5e4a3bdba93850427e6742c4f64c4`, the preceding
binary under the preceding policy is **94 pass / 161 fail / 5 skip**. Applying
the final exact policy to that binary produces **95 pass / 165 fail / 0 skip**.
The repaired runtime under the preceding policy is **255 pass / 0 fail / 5
skip**, and under the final policy is **260/260**. This proves **165**
attributable runtime fail-to-pass transitions with no reverse transition.
Adjacent `%TypedArray%.prototype.reduceRight` remains **50/50**.

The broader `built-ins/Array/prototype/methods-called-as-functions.js`
diagnostic remains outside admission. It now clears reduceRight and fails next
at the independent detached `Array.prototype.reverse` behavior.

Local verification passes all targets/features with **202/202** library
tests, **531/531** builtins tests, and **15/15** arguments tests, plus
**202/202** release library tests, **129/129** Python tooling tests, **1/1**
doctest, rustfmt, warnings-denied Clippy, release build, generated
documentation, and wasm32 checking. Rustdoc retains the 13 pre-existing broken
intra-doc-link warnings.
...
```text
[Decision Log]
- 목적과 의도: Admit complete direct Array reduceRight coverage without weakening broad feature gates or claiming the containing Array prototype directory.
- 기존 구현 및 제약 조건: One hundred sixty-one ordinarily executed files failed, five feature-tagged paths were skipped, and the broader aggregate continues into independent reverse behavior.
- 검토한 주요 대안: Remove TypedArray, resizable-buffer, Reflect, or arrow gates globally; admit the reduceRight directory by prefix; list only former failures; combine this with TypedArray reduceRight or reverse; or freeze all five metadata-bearing paths.
- 선택한 방식: Keep broad gates, add one exact five-path feature map shared by both tools, compare old and repaired binaries on one fixed checkout, and run TypedArray reduceRight as a separate compatibility cohort.
- 다른 대안 대신 이 방식을 선택한 이유: Global and prefix admission silently overclaim unrelated or future behavior, failure-only lists hide existing passes, and combining TypedArray or reverse obscures distinct integer-indexed or in-place mutation semantics. Exact paths keep every policy transition reviewable.
...
## Residual language early errors

Feature commit `f1c421d` closes the five parse-negative failures found by
forcing all **4,452** language parse-negative files through the parser. Three
were labelled generator or async declarations. RuJa previously rejected a
function-like labelled body only in strict code, which accidentally extended
Annex B's ordinary sloppy `FunctionDeclaration` exception to `function*`,
`async function`, and `async function*`. The parser now distinguishes the
ordinary Annex B form from those declaration kinds while preserving the line
terminator boundary after `async`.

The other two files use `aw\u0061it` as a class declaration or expression
name in Module code. The lexer already records the decoded StringValue and
escape presence, but the class-name path did not apply the Module source-goal
restriction. Both raw and escaped `await` are now rejected across the complete
...
`/tmp/ruja-language-early-errors-29903293969-final` aggregate to **31881 pass /
5122 fail / 11461 skip / 3 timeout / 0 error / 48467 total / 37003 run**.
Compared with nullish-ToObject run `29899362112`, 28 result files are
byte-identical. `language/expressions` alone moves from **7858/0/3244** to
**7859/0/3243**, and `language/statements` moves from **4894/0/4443** to
**4898/0/4439**, exactly **+5 pass / -5 skip** overall. The downloaded release
binary reproduces the exact five-file **5/5** result and labelled-directory
**21 pass / 0 fail / 3 skip** result.

```text
[Decision Log]
- 목적과 의도: Close the last forced language parse-negative failures with grammar-correct parser behavior and a closed Test262 support boundary.
- 기존 구현 및 제약 조건: Labelled function-like bodies were rejected only in strict code, so Annex B was overextended to generator and async declarations; escaped class names bypassed the Module-wide await restriction; broad generator, async, class, and module gates still protect unrelated unsupported combinations.
- 검토한 주요 대안: Reject every labelled function declaration; patch only the five Test262 source strings; remove the broad feature and module gates; apply await rejection only at module top level; or admit the files without exact metadata checks.
...
Commit `0740941` changes no Test262 admission. A non-extensible Proxy frame now
reserves its target-key set only after the complete descriptor and omission
pass, while the consumer-filtered result reserves only after each key passes
its String/Symbol and optional enumerable-descriptor checks. Spare vector
capacity, excluded Symbols, absent and non-enumerable descriptors, and empty
lists consume no reservation failure.

Focused regressions cover exact error and fuel priority, successful descriptor
observation before target-set failure, non-extensible mismatch, second actual
filtered-vector growth, partial-state retry, foreign Realms, nested
reverse-frame observations, thrown marker identity, and lazy `for...in`
snapshot and layered-retry atomicity. The selected six-directory local cohort
remains **211 pass / 0 fail / 60 skip / 271 total**.

Local gates pass all targets/features with **230/230** library tests,
**539/539** builtins tests, and **15/15** arguments tests, plus **229/229**
release library tests, **135/135** tooling tests, **1/1** doctest, rustfmt,
warnings-denied Clippy, release build, generated documentation, and wasm32
checking. Rustdoc has only the 13 pre-existing broken-link warnings. The final
GPT-5.6 implementation review is clean. CI `29967042158` and all 33 jobs in
full run `29967042192` pass.

Artifacts at `/tmp/ruja-proxy-ownkeys-post-validation-29967042192-final`
...
run `29963410566`, and the downloaded release binary reproduces the affected
cohort at **211/0/60**.

```text
[Decision Log]
- 목적과 의도: Prove the Proxy ownKeys post-validation allocator correction without changing Test262 admission or mistaking ordinary semantic coverage for host-allocation evidence.
- 기존 구현 및 제약 조건: The selected corpus already passed despite infallible target-set and filtered-vector growth, Test262 cannot inject native reserve failures, and aggregate equality can hide offsetting shard changes.
- 검토한 주요 대안: Admit unrelated files, rely only on focused failpoints, compare aggregate totals only, omit the downloaded artifact check, or broaden this unit to shared strings and GC worklists.
- 선택한 방식: Keep admission unchanged, test every growth and no-growth boundary locally, run the pinned full matrix, compare all 30 artifacts byte-for-byte, and reproduce the selected affected cohort with the downloaded CI binary.
- 다른 대안 대신 이 방식을 선택한 이유: Admission changes do not prove allocation behavior, focused tests do not detect broad build or runner drift, aggregate equality is weaker than file equality, and a local binary does not prove the shipped artifact.
...
`tools/test262_temporal_zoned_date_time_core_admission.txt` freezes **37**
exact paths: 19 constructor-root files, 15 prototype/identifier files, and the
three ZonedDateTime-to-Instant fast-path files. Dedicated CI requires **37 pass
...
is **19/0/1**. The remaining file belongs to the broader civil/formatting
surface.
...
- 검토한 주요 대안: Instant blocker 3개만 허용, ZonedDateTime prefix 전체 허용, constructor/accessor metadata까지 포함한 exact core를 검토했다.
- 선택한 방식: 현재 37개 path와 feature/include/flag/negative metadata를 고정하고 root blocker 1개의 존재·metadata·skip 정책도 검증한다.
- 다른 대안 대신 이 방식을 선택한 이유: fast path만 고정하면 객체 모델 증거가 약하고 prefix admission은 아직 없는 civil formatting을 거짓 지원한다.
- 장점, 단점 및 영향: future sibling은 자동 허용되지 않고 exact 37/0/0, compare 30/0/0, from+equals 62/0/0 경계가 재현된다. 남은 root blocker는 후속 단위에서 재감사한다.
```
...
- 선택한 방식: 현재 통과한 54개 path와 exact metadata를 별도 admission으로 열고 나머지 1개를 blocker manifest로 고정한다. Duration 생성에 의존하던 wrong-type 두 path는 실측 후 이동했다.
- 다른 대안 대신 이 방식을 선택한 이유: prefix 허용은 아직 없는 Temporal types를 거짓 지원하고, 기존 surface 혼합은 method 단위 회귀를 숨긴다. 별도 gate는 54/1 complement와 admission disjointness를 직접 증명한다.
- 장점, 단점 및 영향: future sibling은 자동 허용되지 않고 exact 54/0/0, forced 54/1, fixed 255/11을 재현한다. 다른 Temporal constructors가 추가되면 blocker를 실측 후 이동해야 한다.
```
...
(**3/3**) and full matrix `30879142121` (**59/59**) for the historical
`withTimeZone` **14/2** and equals **52/3** checkpoints.
...
- 기존 구현 및 제약 조건: Temporal feature gate가 전체 directory를 skip했고 현재 한 파일은 아직 없는 다른 Temporal constructors를 선행 생성한다.
- 검토한 주요 대안: 전체 prefix admission, blocker용 가짜 constructors, 기존 fixed-offset manifest 혼합, exact admission과 complement를 검토했다.
- 선택한 방식: 현재 통과한 15개 path와 live metadata를 runner/analyzer 공유 manifest로 열고 1개 blocker를 forced diagnostic으로 고정한다.
...
- 장점, 단점 및 영향: exact 15/0/0과 forced 15/1이 재현되며 future sibling은 자동 허용되지 않는다. calendar-bearing Temporal types 도입 시 blocker를 실측 후 이동해야 한다.
```

Implementation commit `86124ef` is confirmed by ordinary CI `30906827851`
(**3/3**) and full matrix `30906827947` (**60/60**) for the historical
`withCalendar` **14/2** checkpoint.
